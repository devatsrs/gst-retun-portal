  ...
  function (
    ...
    function initVersionCheck() {
      if ($scope.checknewVersionFurther) {
        g1FileHandler.versionCheck().then(
          function (response) {
            if (
              response.gst_offline_tool &&
              response.gst_offline_tool.current_version
            ) {
    ...
    $scope.openDownloadWindow = function () {
      if (window.ActiveXObject || "ActiveXObject" in window) {
        ...
        if (shell) shell.run($scope.download_url);
      } else {
    ...
    function initStateLs() {
      g1FileHandler.getStateList().then(
        function (response) {
    ...
    function initReasonLs() {
      g1FileHandler.getReasonList().then(
        function (response) {
    ...
    function initRateList() {
      g1FileHandler.getRateList().then(
        function (response) {
    ...
    function initInvTypeList() {
      g1FileHandler.getInvTypes().then(
        function (response) {
...
myApp.controller("gstrhomectrl", [
  "$scope",
  "shareData",
  "g1FileHandler",
  "$log",
  function ($scope, shareData, g1FileHandler, $log) {
    ...
    shareData.disableAATOLengthCheck =
      GSTR1_HSN_AATO_LENGTH_CHECK_FLAG == "N" ? true : false;
    shareData.disableHSNRestrictions =
      GSTR1_HSN_DESC_CHECK_FLAG == "N" ? true : false;
    ...
    $scope.onErrorFileLoad = function (iFileData) {
      ...
      if (!FileSelectedcheck(tmp_name, ".zip")) {
        $scope.createAlert(
          "WarningOk",
          "Selected File type is not allowed. Please upload zip file",
          function () {
      ...
      g1FileHandler.uploadErrFile(fd).then(
        function (response) {
          ...
          if (sessionStorage) {
            if (response && response.data && response.data.length == 1) {
              ...
              sessionStorage.setItem(
                "fileNameData",
                response.data[0].fp + "_" + response.data[0].gstin
              );
    ...
    $scope.onImportReturnFileLoad = function (iFileData) {
      ...
      for (var i = 0, len = iFileData.length; i < len; i++) {
        ...
        if (!FileSelectedcheck(tmp_name, ".zip")) {
          $scope.createAlert(
            "WarningOk",
            "Selected File type is not allowed. Please upload zip files",
            function () {
              //$scope.dashboard.$submitted = false;
              //$scope.page("/gstr/error/dashboard");
            }
      ...
      g1FileHandler.uploadReturnFile(fd).then(
        function (response) {
    ...
    $scope.handleFilecall = function (iFileData) {
      ...
      for (var i = 0, len = iFileData.length; i < len; i++) {
        ...
        if (!FileSelectedcheck(tmp_name, ".json")) {
          $scope.createAlert(
            "WarningOk",
            "Selected File type is not allowed. Please upload json files",
            function () {}
          );
          return false;
        }
      ...
      g1FileHandler.mstrUploadFile(fd).then(
        function (response) {
...
myApp.controller("prvctrl", [
  ...
  function ($scope, shareData, g1FileHandler, $log, NgTableParams, $filter) {
    ...
    function initPreview() {
      ...
      g1FileHandler.getSectionList(shareData.dashBoardDt.form).then(
        function (response) {
          sectionLs = response;
          ...
          reformSummary(response, function (iData) {
            ...
            if (shareData.isTPQ && rtn_prd % 3 !== 0) {
              ...
              for (a; a < aLen; a++) {
                if (
                  $scope.previewModel[a].cd == "b2b" ||
                  $scope.previewModel[a].cd == "b2ba" ||
                  $scope.previewModel[a].cd == "cdnr" ||
                  $scope.previewModel[a].cd == "cdnra"
                ) {
    ...
    $scope.isGstr2Prvw = function () {
      return $scope.dashBoardDt && $scope.dashBoardDt.form == "GSTR2"
        ? true
        : false;
    };
    ...
    function reformSummary(iResp, callback) {
      g1FileHandler.getContentsForMeta(shareData.dashBoardDt).then(
    ...
    $scope.generateZip = function () {
      var reqParam = shareData.dashBoardDt;
      g1FileHandler.createZip(reqParam).then(
        function (response) {
          ...
          saveAs(
            ...
            shareData.dashBoardDt.form +
              "_" +
              shareData.dashBoardDt.gstin +
              "_" +
              shareData.dashBoardDt.month +
              "_" +
              shareData.dashBoardDt.fy +
              ".zip"
          );
        },
        function (error) {
    ...
    $scope.generateReturn = function () {
      ...
      var reqParam = shareData.dashBoardDt;
      $().blockPage(true);
      g1FileHandler.createFile(reqParam).then(
        function (response) {
          $().blockPage(false);
...
myApp.controller("upldmstrprvctrl", [
  "$scope",
  "shareData",
  "g1FileHandler",
  "$log",
  ...
  function ($scope, shareData, g1FileHandler, $log, NgTableParams, $filter) {
    ...
    $scope.initmstrPreview = function () {
      console.log(
        "$scope.mstrdashBoardDt.productsMasters",
        $scope.mstrdashBoardDt.productsMasters
      );
...
myApp.controller("uploadprvctrl", [
  "$scope",
  "shareData",
  "g1FileHandler",
  "$log",
  ...
  function ($scope, shareData, g1FileHandler, $log, $filter, R1Util) {
    ...
    function initPreview() {
      ...
      shareData.dashBoardDt.form;
      g1FileHandler.getUploadSectionList(shareData.dashBoardDt.form).then(
        function (response) {
          sectionLs = response;
    ...
    function getUploadedSectionsOnly(iResp) {
      g1FileHandler.getUploadContentsFor(shareData.dashBoardDt).then(
        function (prevContent) {
          $log.debug(
            "uploadsummarycrtl -> getUploadContentsFor success:: ",
            prevContent
          );
        ...
        function (response) {
          $log.debug(
            "uploadsummarycrtl -> getUploadContentsFor Failure:: ",
            response
          );
        }
      );
    ...
    function generateTotal(total, itmInner, iSec) {
      if (iSec == "itc_rcd" && form == "GSTR2") {
        (itmInner.camt = parseFloat(itmInner.o_cg) + parseFloat(itmInner.n_cg)),
          (itmInner.iamt =
            parseFloat(itmInner.o_ig) + parseFloat(itmInner.n_ig)),
          (itmInner.samt =
            parseFloat(itmInner.o_sg) + parseFloat(itmInner.n_sg)),
          (itmInner.csamt =
            parseFloat(itmInner.o_cs) + parseFloat(itmInner.n_cs));
      }
    ...
    function reformSummary(iResp, callback) {
      ...
      if (tmpFrm == "GSTR2A" || tmpFrm == "GSTR1IFF") tmpFrm = "GSTR1";
      g1FileHandler.getContentsForMeta(shareData.dashBoardDt, null, true).then(
        function (prevContent) {
    ...
    $scope.saveData = function (fileName) {
      ...
      g1FileHandler.getDownloadContentsFor(fName).then(
        function (response) {
          $log.debug("prvctrl -> file generated :: ", response);
          saveAs(
            ...
            "returns" +
              ...
              date +
              "_" +
              shareData.dashBoardDt.form.substring(3) +
              "_" +
              shareData.dashBoardDt.gstin +
              "_" +
              "offline" +
    ...
    $scope.isGSTR2A = function () {
      return $scope.dashBoardDt && $scope.dashBoardDt.form == "GSTR2A"
        ? true
        : false;
    };
    $scope.isGSTR1A = function () {
      return $scope.dashBoardDt && $scope.dashBoardDt.form == "GSTR1A"
        ? true
        : false;
    };
    ...
    $scope.isGSTR1 = function () {
      return ($scope.dashBoardDt && $scope.dashBoardDt.form == "GSTR1") ||
        $scope.dashBoardDt.form == "GSTR1IFF"
        ? true
        : false;
    };
    ...
    $scope.generateExcel = function () {
      ...
      reqParam.type = "Import";
      $().blockPage(true);
      g1FileHandler.createExcel(reqParam).then(
        function (response) {
          $log.debug("uploadprvctrl -> file generated :: ", response);
          saveAs(
            new Blob([response], {
...
myApp.controller("downloadctrl", [
  ...
  function ($scope, shareData, g1FileHandler, $log, $filter) {
    if (!shareData.dashBoardDt) {
      if ((shareData.flowName = "New")) $scope.page("/gstr/summary");
    ...
    $scope.goToBackPage = function () {
      if (shareData.flowName == "New") $scope.page("/gstr/preview");
      else if (shareData.flowName == "Import")
        $scope.page("/gstr/upload/preview");
      else $scope.page("/gstr/error/preview");
    };
  },
]);

myApp.controller("errPrvCtrl", [
  "$scope",
  "shareData",
  "g1FileHandler",
  "R1Util",
  "$log",
  "$filter",
  function ($scope, shareData, g1FileHandler, R1Util, $log, $filter) {
    if (!shareData.dashBoardDt) {
      $scope.page("/gstr/error/summary");
    } else {
      $scope.dashBoardDt = shareData.dashBoardDt;
    ...
    function initPreview() {
      ...
      g1FileHandler.getErrorSectionList(shareData.dashBoardDt.form).then(
        function (response) {
      ...
      function getErrorSectionsOnly(iResp) {
        g1FileHandler.getErrorContentsFor(shareData.dashBoardDt).then(
          function (prevContent) {
            $log.debug(
              "errprvctrl -> getErrorContentsFor success:: ",
              prevContent
            );
          ...
          function (response) {
            $log.debug(
              "errorsummaryctrl -> getErrorContentsFor Failure:: ",
              response
            );
          }
        );
    ...
    function generateItcTotal(total, itcInner, iSec) {
      total.itc_cgTl += itcInner.tx_c
        ? parseFloat(itcInner.tx_c.toFixed(2))
        : 0;
      total.itc_sgTl += itcInner.tx_s
        ? parseFloat(itcInner.tx_s.toFixed(2))
        : 0;
      total.itc_igTl += itcInner.tx_i
        ? parseFloat(itcInner.tx_i.toFixed(2))
        : 0;
      total.itc_csTl += itcInner.tx_cs
        ? parseFloat(itcInner.tx_cs.toFixed(2))
        : 0;
        ...
        function (prevContent) {
          ...
          for (var a = 0, alen = iResp.length; a < alen; a++) {
            ...
            var count = 0,
              ...
              result = {},
              total = null;
            if (typeof ctinInv === "undefined") continue;
            if (shareData.dashBoardDt.form == "GSTR1") {
              total = {
                cgTl: 0,
                sgTl: 0,
            ...
            if (shareData.dashBoardDt.form == "GSTR1") {
              switch (section.cd) {
                ...
                case "hsn":
                  if (ctinInv)
                    if (
                      !R1Util.isCurrentPeriodBeforeAATOCheck(
                        shareData.newHSNStartDateConstant,
                        shareData.monthSelected.value
                      )
                    ) {
                      angular.forEach(ctinInv, function (itmInner, i) {
    ...
    $scope.generateCorrectedErrorJson = function () {
      var reqParam = shareData.dashBoardDt;
      $().blockPage(true);
      g1FileHandler.createErrorFile(reqParam).then(
        function (response) {
          $().blockPage(false);
...
myApp.controller("dashboardcrtl", [
  "$scope",
  "g1FileHandler",
  "$log",
  "$location",
  ...
  function ($scope, g1FileHandler, $log, $location, shareData) {
    ...
    $scope.isQTaxpayer = function () {
      var rtn_prd = parseInt(
        $scope.monthSelected.value.slice(2) +
          $scope.monthSelected.value.slice(0, 2)
      );
    ...
    function initDashboard() {
      $scope.initInfo(null, null, null, null);
      //Get Dropdown list
      g1FileHandler.getDropdown().then(
        function (response) {
          ...
          if (prd >= $scope.iffstrtprd) loadForms(true);
          else loadForms(false);
        },
        function (response) {
    ...
    $scope.isCurrentPeriodBeforeAATOCheck = function (
      start_period,
      current_period
    ) {
    ...
    function loadForms(isQtp) {
      //get GSTR Forms
      g1FileHandler.getGstForms(isQtp).then(
        function (response) {
          $log.debug("dashboardcrtl -> getDropdown success:: ", response);
          ...
          angular.forEach($scope.GstrForms, function (form, index) {
            if (form.cd == "GSTR1A") flag = index;
          });
          $scope.GstrForms.splice(flag, 1);

          if (
            shareData.dashBoardDt &&
    ...
    $scope.fileReturns = function () {
      ...
      if ($scope.dashboard.$valid) {
        ...
        var insertTemplate = function () {
          //CR#18639- IFF changes by prakash
          let frm;
          if (reqParam.form == "GSTR1IFF") frm = "GSTR-1/IFF";
          else frm = reqParam.form;
          if (isNaN(reqParam.gt) && isNaN(reqParam.cur_gt)) {
            //CR#18639- IFF confirmation popup
            var temp1 =
              "<br/><b>GST Statement/Returns: </b>" +
              frm +
              "\
                        <br/><b>GSTIN of Supplier: </b>" +
              reqParam.gstin +
              "\
                        <br/><b>Financial Year: </b>" +
              reqParam.fy +
              "\
                        <br/><b>Tax Period: </b>" +
              reqParam.month +
              "\
                        <br/><b>Are you a SEZ Taxpayer? </b>" +
              sezcnfrm;
            ...
            if (
              ...
              var temp2 =
                "<br/><b>Is taxpayer Quarterly filer? </b>" + qtaxpayercnfrm;
              //CR # 19427
              if (
                !shareData.isTPQ &&
                !$scope.isCurrentPeriodBeforeAATOCheck(
                  shareData.newHSNStartDateConstant,
                  $scope.monthSelected.value
                )
              ) {
                temp2 +=
                  "<br/><b>Is aggregate turnover greater than 5CR in preceding financial year? </b>" +
                  aatogreaterthan5CR;
              }
              return temp1 + temp2;
            }
            ...
            return (
              ...
              sezcnfrm +
              "\
									<br/><b>Aggregate Turnover in the preceding Financial Year: ₹</b>" +
              reqParam.gt +
              "\
									<br/><b>Aggregate Turnover- April to June, 2017: ₹</b>" +
              reqParam.cur_gt
            );
          }
        };
        ...
        $scope.createAlert(
          "Warning",
          "<b><center>Please confirm your details before you proceed</center></b>" +
            insertTemplate(),
          function () {
            $scope.page("/gstr/summary");
          }
    ...
    $scope.isGstr1 = function () {
      return $scope.formNum === "GSTR1" || $scope.formNum === "GSTR1IFF"
        ? true
        : false;
    };
...
myApp.controller("uploaddashboardctrl", [
  "$scope",
  "g1FileHandler",
  "$log",
  "$location",
    ...
    function loadForms(isQtp) {
      ...
      g1FileHandler.getGstForms(isQtp).then(
        function (response) {
          ...
          //CR#18639- IFF changes by prakash
          if (isQtp && fileNameDetails[2] == "R1") $scope.formNum = "GSTR1IFF";
          else $scope.formNum = "GST" + fileNameDetails[2];
        },
        function (response) {
          $log.debug("dashboardcrtl -> getDropdown fail:: ", response);
        }
...
myApp.controller("upldmstrdashboardctrl", [
  "$scope",
  "g1FileHandler",
  "$log",
  "$location",
  ...
  function ($scope, g1FileHandler, $log, $location, shareData) {
    ...
    $scope.getmstrUploadedDetails = function () {
      if (
        $scope.validations.gstin($scope.userGstin) &&
        $scope.mstrdashboard.$valid
      ) {
        var reqParam = {
...
myApp.controller("masterdashboardctrl", [
  "$scope",
  "g1FileHandler",
  "$log",
  "$location",
  ...
  function ($scope, g1FileHandler, $log, $location, shareData) {
    ...
    $scope.onGstinChange = function () {
      var validGstin =
        $scope.validations.gstin($scope.userGstin) &&
        !$scope.validations.tdsregex($scope.userGstin) &&
        !$scope.validations.tcsregex($scope.userGstin) &&
        !$scope.validations.oidarregex($scope.userGstin) &&
        !$scope.validations.nriregex($scope.userGstin) &&
        !$scope.validations.unibodyregex($scope.userGstin);
...
myApp.controller("errordashboardctrl", [
  "$scope",
  "g1FileHandler",
  "$log",
  "$location",
  ...
  function ($scope, g1FileHandler, $log, $location, shareData) {
    ...
    $scope.getUploadedDetails = function () {
      if ($scope.dashboard.$valid) {
        ...
        $scope.createAlert(
          "Warning",
          "<b><center>Please confirm details before you proceed</center></b>" +
            insertTemplate(),
          function () {
            $scope.page("/gstr/error/preview");
          }
...
myApp.controller("summarycrtl", [
  ...
  function (
      ...
      g1FileHandler.getSectionList(shareData.dashBoardDt.form).then(
        function (response) {
          ...
          if (response) {
            ...
            $scope.sectionListSelected =
              $scope.sectionList[shareData.scLsSetIdx];
          }
    ...
    $scope.deleteReturn = function () {
      ...
      g1FileHandler.deleteReturn(param).then(
        function (response) {
          ...
          if (
            ...
            $scope.sectionListSelected.url =
              "pages/returns/" + formName + "/" + cd + "/newSummary.html";
          } else {
            $scope.sectionListSelected.url =
              "pages/returns/" + formName + "/" + cd + "/summary.html";
          }
        },
        function (error) {
    ...
    $scope.deleteSectionData = function () {
      ...
      g1FileHandler.deleteSectionData(param).then(
        function (response) {
...
myApp.controller("mastersummarycrtl", [
  "$scope",
  "$rootScope",
  "g1FileHandler",
  "$log",
  ...
  function (
    ...
    $scope.initmastersummary = function () {
      ...
      g1FileHandler.getMasterData(shareData.userGstin).then(
        function (response) {
          $scope.savemstr =
      ...
      if (event && event !== "") {
        ...
        $scope.savemstr = $scope.orgmodifiedData.filter(function (element1) {
          if (element1 !== undefined && element1 !== null && event) {
            if (element1.uqc !== "") {
              return (
                element1.prdname.toLowerCase().indexOf(event.toLowerCase()) >=
                  0 ||
                element1.hsndesc.toLowerCase().indexOf(event.toLowerCase()) >=
                  0 ||
    ...
    $scope.hsnBlurvalidation = function () {
      if (
        !$scope.hsnsaveedit &&
        $scope.newmstr.hsn_sc &&
        $scope.newmstr.hsn_sc.length > 0
      ) {
    ...
    $scope.getSummary = function (summaryData) {
      var result = summaryData.reduce(function (acc, o) {
        acc[o.igstrt] = (acc[o.igstrt] || 0) + 1;
        return acc;
      }, {});
      var sum = summaryData.reduce(function (sum) {
        sum = sum + 1;
        return sum;
      }, 0);
        ...
        function () {
          ...
          if (index != -1) {
            ...
            g1FileHandler
              .saveProductMstr(
                shareData.userGstin,
                deleteunwantedparams($scope.savemstr),
                "productsMasters"
              )
              .then(function (response) {
                $scope.successMessageShow = true;
    ...
    $scope.updateProd = function (index) {
      ...
      angular.forEach($scope.orgmodifiedData, function (mstrprd, i) {
        if (
          mstrprd.prdname &&
          mstrprd.prdname.toLowerCase() ==
            $scope.savemstr[index].prdname.toLowerCase()
        ) {
          orgEditIndex = i;
        }
      });
      ...
      if ($scope.view == "product") {
        ...
        if ($scope.savemstr.length < 1) {
          ...
          $scope.createAlert(
            ...
            function () {
              g1FileHandler
                .saveProductMstr(shareData.userGstin, [], tabMaster)
                .then(function (response) {
                  $scope.successMessageShow = true;
                  window.scroll(0, 0);
                  $timeout(function () {
...
myApp.controller("uploadsummaryctrl", [
  "$scope",
  "$rootScope",
  "g1FileHandler",
  "$log",
  ...
  function (
    ...
    function initFormatters() {
      reformateInv = ReturnStructure.reformateInv(
        $scope.suplyList,
        shareData.dashBoardDt.gstin,
        $scope.sectionListSelected.cd,
        shareData.dashBoardDt.form
      );
      formateNodePayload = ReturnStructure.formateNodePayload(
        $scope.sectionListSelected.cd,
        shareData.dashBoardDt.form
      );
    }
    ...
    $scope.onSectionChange = function (iData) {
      ...
      if (form == "GSTR1" || form == "GSTR1IFF") $scope.isUploadFlag = "S";
      else $scope.isUploadFlag = "R";
    ...
    $scope.deleteSectionData = function () {
      ...
      g1FileHandler.deleteSectionData(param).then(
        function (response) {
          $log.debug(
            "uploadsummarycrtl -> deleteSectionData succ :: ",
            response
          );
          $scope.createAlert("Success", response);
          var cd = $scope.sectionListSelected.cd;
          ...
          $scope.sectionListSelected.url =
            "/pages/returns/upload/" + form + "/" + cd + "/" + summaryUrl;
        },
        function (error) {
    ...
    $scope.setDeleteFlagForData = function () {
      $scope.createAlert(
        "Warning",
        "Data of selected section will be deleted from Online Portal upon upload of generated JSON",
        function () {
          $().blockPage(true);
          g1FileHandler.uploadDeleteAll($scope.dashBoardDt).then(
            function (response) {
              ...
              if (response) {
                ...
                if (
                  $scope.sectionListSelected.cd == "b2b" ||
                  $scope.sectionListSelected.cd == "cdnr" ||
                  $scope.sectionListSelected.cd == "b2ba" ||
                  $scope.sectionListSelected.cd == "cdnra"
                ) {
                  $scope.sectionListSelected.url =
                    "pages/returns/upload/" +
                    $scope.dashBoardDt.form.toLowerCase() +
                    "/" +
                    $scope.sectionListSelected.cd +
                    "/uploadedbytaxpayersummary.html";
                } else {
            ...
            function (response) {
              ...
              $().blockPage(false);
              $log.debug(
                "returnsctrl -> setDeleteFlagForData fail:: ",
                response
              );
            }
          );
    ...
    $scope.isB2BCDNR = function () {
      return $scope.sectionListSelected &&
        ($scope.sectionListSelected.cd == "b2b" ||
          $scope.sectionListSelected.cd == "cdnr" ||
          $scope.sectionListSelected.cd == "b2ba" ||
          $scope.sectionListSelected.cd == "cdnra")
    ...
    function getUrl(iFlag) {
      ...
      } else if (form == "GSTR1A") summaryUrl = "counterpartysummary.html";
      else {
        if (iFlag == "R") {
          summaryUrl = "uploadedbytaxpayersummary.html";
        } else if (iFlag == "S") {
...
myApp.controller("errorsummaryctrl", [
  "$scope",
  "$rootScope",
  "g1FileHandler",
  "$log",
  ...
  function (
    ...
    function getErrorSectionList() {
      g1FileHandler.getErrorSectionList(shareData.dashBoardDt.form).then(
        function (response) {
          $log.debug(
            "errorsummaryctrl -> getErrorSectionList success:: ",
            response
          );
          if (response) {
            getErrorSectionsOnly(response);
        ...
        function (response) {
          $log.debug(
            "errorsummaryctrl -> getErrorSectionList fail:: ",
            response
          );
        }
      );
    ...
    function getErrorSectionsOnly(iResp) {
      ...
      g1FileHandler.getErrorContentsFor(shareData.dashBoardDt).then(
        function (prevContent) {
          $log.debug(
            "errorsummaryctrl -> getErrorContentsFor success:: ",
            prevContent
          );
        ...
        function () {
          ...
          setTimeout(function () {
            if ($scope.processStarted) {
              ...
              $("body").append(
                '<div class="delayMSg" style="position: fixed; z-index: 99999; bottom: 0; left: 0; right: 0; width: 100%; height: 40px; text-align: center; background: white; padding-top: 11px; border-top: 1px solid #2c4e86;">Import is being processed, please wait...</div>'
              );
            }
          }, 1000);
          ...
          var file = $scope.excelFile;
          g1FileHandler
            .savePayloadFile(file, shareData)
            .then(function (response) {
    ...
    $scope.onExcelProcessed = function (data) {
      R1InvHandler.upMultipleSections(
        $scope,
        { cache_key: data.cache_key },
        data.errData,
        data.multiItmErrData,
        data.misMatchedErrData,
        data.errb2clInvAry,
        data.errShipDtData,
        data.errActionInvAry,
        data.errActionErrorStatusInvAry,
        data.errMissingHeaderDataArry,
        shareData.isUploadImport
      )
        .then(
          function (iRs) {
            $(".delayMSg").remove();
  ...
  function (
    ...
    if (shareData.dashBoardDt) {
      ...
      if (shareData.isUploadImport == "Y") {
        //if it's Y it's for second flow
        $scope.page("/gstr/upload/summary");
      ...
      g1FileHandler.getSectionList(shareData.dashBoardDt.form).then(
        function (response) {
          ...
          if (
            shareData.isTPQ == false &&
            rtn_prd % 3 === 0 &&
            $scope.sectionList[5].nm == "Nil Rated Invoices"
          ) {
            $scope.sectionList[5].disabled = true;
          }
          //CR#18639-changes for IFF_form
    ...
    $scope.onExcelProcessed = function (data) {
      ...
      if (data.validFlag !== undefined && !data.validFlag) {
        $scope.createAlert(
          "Error",
          "Based on your filing preference (quarterly), you are only allowed to import details for B2B, CDNR, B2BA and CDNRA tables for selected period."
        );
        $(".delayMSg").remove();
        $().blockPage(false);
      } else {
        R1InvHandler.upMultipleSections(
          ...
          data.errMissingHeaderDataArry,
          shareData.isUploadImport,
          data.errUrTypePosData,
          data.errUrtypeDiffData,
          data.errPosStCdData,
          data.errInvSplyType
        )
          .then(
          ...
          angular.forEach(iExcelData, function (section, i) {
            if (section.section != "Help Instruction") {
              var secNm = section.section,
                ...
                invNodeFormatter = ReturnStructure.formateNodePayload(
                  secNm,
                  form
                ), //formateB2BNodePayload
              ...
              var preparedExcelPayload = R1InvHandler.preparePayloadFromExcel(
                  secDt,
                  invStructure,
                  invItmStructure,
                  excelInvTitle,
                ...
                misMatchedPatternInvAry =
                  preparedExcelPayload.excelInvldPattrnList,
                errTaxRtInvAry = preparedExcelPayload.excelb2clErrList,
                errInvalidActionAry =
                  preparedExcelPayload.excelInvalidActionList;
              ...
              misMatchedErrData = misMatchedErrData.concat(
                misMatchedPatternInvAry
              );
              errTaxRtInvData = errTaxRtInvData.concat(errTaxRtInvAry);
              errInvalidActionData =
                errInvalidActionData.concat(errInvalidActionAry);
            }
          });
    ...
    $scope.onpastedone = function (iData, type) {
      ...
      var sectionId = $scope.sectionListSelected.cd,
        ...
        getExcelTitle = ReturnStructure.getExcelTitle(
          sectionId,
          form,
          $scope.trans
        ),
        ...
        function () {
          ...
          if (
            ...
            if (shareData.disableHSNRestrictions) {
              ...
              angular.forEach(iData, function (sInv, i) {
                var ajaxCall = ReturnStructure.fetchDescFromHsn(sInv.HSN).then(
                  function (response) {
            ...
            $q.all(promises).then(function () {
              var preparedExcelPayload = R1InvHandler.preparePayloadFromExcel(
                ...
                UrTypPOSErrList =
                  preparedExcelPayload.excelInvalidURtypePOSList,
                UrTypDiffErrList =
                  preparedExcelPayload.excelInvalidURtypeDiffPerList,
                PosSupStCdErrList =
                  preparedExcelPayload.excelInvalidPosSupStCode,
                NoteSplyTpeErrList =
                  preparedExcelPayload.excelInvalidNtSplyTypList;

              angular.forEach(invAry, function (sInv, i) {
                newInvAry1.push(formateNodePayload(sInv));
              });
              R1InvHandler.onPaste(
                $scope,
                newInvAry1,
                sectionId,
                errInvAry,
...
myApp.controller("nilsumryctrl", [
  ...
  function (
    ...
    $scope.isSEZIntra = function (iObj) {
      return shareData.isSezTaxpayer &&
        (iObj.sply_ty == "INTRAB2B" || iObj.sply_ty == "INTRAB2C")
        ? true
        : false;
    };
...
myApp.controller("nilsumry2ctrl", [
  "$scope",
  "$rootScope",
  "shareData",
  "g1FileHandler",
    ...
    function initnilSumryList() {
      g1FileHandler
        ...
        .then(
          function (response) {
            $log.debug(
              "nilsumry2ctrl -> initnilSumryList success:: ",
              response
            );
...
myApp.controller("itcrvrslctrl", [
  ...
  function (
    ...
    $scope.chckItcForm = function (data) {
      var keys = Object.keys($scope.itcSumryList),
        ct = 0;
      if (data == undefined || JSON.stringify(data) == "{}") {
        ...
        for (var i in keys) {
          if (
            !data[keys[i]] ||
            (data[keys[i]] &&
              (data[keys[i]].iamt == "" || data[keys[i]].iamt == undefined) &&
              (data[keys[i]].samt == "" || data[keys[i]].samt == undefined) &&
              (data[keys[i]].camt == "" || data[keys[i]].camt == undefined))
          ) {
            ct++;
          }
        }
    ...
    $scope.saveItcReversalSumryList = function () {
      ...
      if (
        $scope.itcrvslPage.$valid &&
        !$scope.chckItcForm($scope.itcSumryList)
      ) {
        ...
        if (stdata) {
          ...
          for (var i = 0; i < m.length; i++) {
            stdata[m[i]].iamt = stdata[m[i]].iamt
              ? parseFloat(stdata[m[i]].iamt)
              : 0;
            stdata[m[i]].csamt = stdata[m[i]].csamt
              ? parseFloat(stdata[m[i]].csamt)
              : 0;
            stdata[m[i]].camt = stdata[m[i]].camt
              ? parseFloat(stdata[m[i]].camt)
              : 0;
            stdata[m[i]].samt = stdata[m[i]].samt
              ? parseFloat(stdata[m[i]].samt)
              : 0;
