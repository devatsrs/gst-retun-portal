...
myApp
  ...
  .controller("headerCtrl", [
    ...
    "shareData",
    "$rootScope",
    function ($scope, ajax, $timeout, shareData, $rootScope) {
      $scope.udata = undefined;
      $scope.servers = $().getServers();
      ajax.get("/services/api/ustatus").then(
        function (resp) {
...
myApp.config([
  ...
  function ($httpProvider, $compileProvider, localStorageServiceProvider) {
...
myApp.config([
  ...
  "$logProvider",
  "envServiceProvider",
  function ($provide, $logProvider, envServiceProvider) {
...
myApp.service("validations", function () {
  this.formats = {
    ...
    tdsnormalgstin:
      /([0-9]{2}[a-zA-Z]{5}[0-9]{4}[a-zA-Z]{1}[1-9A-Za-z]{1}[Zz1-9A-Ja-j]{1}[0-9a-zA-Z]{1})|([0-9]{2}[a-zA-Z]{4}[a-zA-Z0-9]{1}[0-9]{4}[a-zA-Z]{1}[1-9A-Za-z]{1}[D]{1}[0-9a-zA-Z]{1})/,
  ...
  this.messages = {
    ...
    svtax:
      "Invalid entry, Please enter 6-25 character alphanumeric service tax registration number",
...
myApp.service("ajax", [
  ...
  "httpLoader",
  "$route",
  function ($http, $q, $rootScope, httpLoader, $route) {
...
myApp.directive("errorDialogueCallback", function () {
  return {
    ...
    template:
      '<div id="errorCllbackDlg" data-backdrop="static" class="modal fade fade-scale" role="dialog"><div class="modal-dialog sweet"><div class="modal-content"><div class="modal-body"><div class="m-icon m-error"><span class="x-mark"><span class="m-line m-left"></span><span class="m-line m-right"></span></span></div><h2>{{title}}</h2><p ng-bind-html="message|trust"></p></div><div class="modal-footer"><button class="btn btn-default" ng-click="callback()">{{okTitle}}</button></div></div></div></div>',
  };
});
...
myApp.directive("alertMessage", function () {
  return {
    ...
    template:
      '<div class="alert alert-{{type}}"><a class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>{{title}}</strong> {{message}}.</div>',
  };
});
...
myApp.directive("warnDeleteDialogue", function () {
  return {
    ...
    template:
      '<div id="warDelteDlg" data-backdrop="static" class="modal fade fade-scale" role="dialog"><div class="modal-dialog sweet"><div class="modal-content"><div class="modal-body"><div class="m-icon m-warning pulseWarning" style="border-color: #2c4e86;"><span class="micon-body pulseWarningIns" style="background-color: #2c4e86;"></span><span class="micon-dot pulseWarningIns" style="background-color: #2c4e86;"></span></div><h2>{{title}}</h2><p ng-bind-html="message|trust"></p></div><div class="modal-footer"><button class="btn btn-default" data-dismiss="modal" ng-if="cancelTitle!==\'null\'">{{cancelTitle}}</button><button autofocus class="btn btn-primary" ng-click="callback()">{{okTitle}}</button></div></div></div></div>',
  };
});
...
myApp.factory("utilFunctions", [
  ...
  "$rootScope",
  "$filter",
  function ($q, $log, cache, version, $rootScope, $filter) {
    return {
      initCache: function () {
      ...
      initLanguage: function (module) {
        ...
        if (navigator.cookieEnabled) {
          if (
            cache.get(module + ($().getCookie("Lang") || "en")).length === 0
          ) {
          ...
          if (
            $rootScope.langStore &&
            $rootScope.langStore[module + default_lang]
          ) {
      ...
      createDialogue: function (config) {
        $(
          "confirm-dialogue,error-dialogue,success-dialogue,error-dialogue-callback,info-dialogue,warn-dialogue,warn-delete-dialogue"
        ).remove();
        ...
        switch (config.type) {
          case "Warning":
            return (
              ...
              config.ok_btn_title +
              "' data-cancel-title='" +
              config.cancel_btn_title +
              "'></confirm-dialogue>"
            );
          case "Error":
            return (
              "<error-dialogue data-title='" +
              config.title +
              "' data-message='" +
              config.message +
              "' data-cancel-title='" +
          ...
          case "ErrorCallback":
            return (
              "<error-dialogue-callback data-title='" +
              config.title +
              "' data-message='" +
              config.message +
              "'   data-callback=\"" +
          ...
          case "WarnDelete":
            return (
              "<warn-delete-dialogue data-title='" +
              config.title +
              "' data-message='" +
              config.message +
              "' data-callback=\"" +
              ...
              "' data-cancel-title='" +
              config.cancel_btn_title +
              "'></warn-delete-dialogue>"
            );
        }
      },
      _destroyDialogue: function () {
        var x = $("confirm-dialogue,error-dialogue,success-dialogue").find(
          ".modal"
        );
        if (x.length > 0) {
      ...
      createDeleteDialogue: function (config) {
        return (
          "<delete-dialogue data-warning-title='" +
          ...
          config.callback +
          '()" ' +
          'data-warning-callback="' +
          config.callback +
          "()\" data-warning-ok-title='" +
          config.warning.ok_btn_title +
          "' data-warning-cancel-title='" +
          config.warning.cancel_btn_title +
          "'></delete-dialogue>"
        );
      },
          ...
          conn.onmessage = function (e) {
            ...
            $.map(em_data, function (i) {
              ...
              if (xd.length > 0) {
                if (
                  (xd[0].trim() == "version" &&
                    xd[1].trim() == $().getConstant("LATEST_EM_VERSION")) ||
                  (xd[0].trim() == "ID" &&
                    xd[1].trim() == $().getConstant("EM_ENTY_ID"))
                ) {
                  verfy_length++;
                }
              }
...
function transctrl(
  ...
  $scope.loadLang = function () {
    ajax.get("/lang/languages.json").then(
      function (data) {
  ...
  $scope.selectLang = function (lang) {
    ...
    if (navigator.cookieEnabled) {
      ...
      if (linStore.length === 0 || !linStore) {
        module = this.module;
        translationSrv.getTranslation(lang, module).then(
          function (d) {
            if (d && d !== "" && d !== undefined) {
              $scope.selectedLang = $scope.getLangTitle(
                cache.get("languages"),
                lang
              );
              $scope.trans = d;
      ...
      if (
        $rootScope.langStore &&
        $rootScope.langStore.length !== 0 &&
        $rootScope.langStore.indexOf(this.module + lang)
      ) {
        ...
        $rootScope.langStore = [];
        translationSrv.getTranslation(lang, this.module).then(
          function (d) {
            if (d && d !== "" && d !== undefined) {
              $scope.trans = d;
              $scope.selectedLang = $scope.getLangTitle(
...
myApp.run([
  ...
  "ajax",
  "$route",
  function ($rootScope, ngProgressFactory, ajax, $route) {
...
myApp.directive("photoModel", [
  "ajax",
  "$q",
  "$parse",
  "cameraAPI",
  function (ajax, $q, $parse, cameraAPI) {
    function link(scope, element, attrs) {
      ...
      scope.openCamera = function () {
        ...
        var stream = cameraAPI.openCamera(
          "video" + scope.id,
          "canvas" + scope.id
        );
        stream.then(
    ...
    return {
      ...
      template: function (attrs) {
        var output =
          ...
          "<button ng-click='openCamera()' title='Take Picture' type='button' class='btn btn-primary' data-ng-if='(videoStreams | numkeys).length == 0 && !captured'><i class='fa fa-camera'></i> Take Picture</button>" +
          "<p data-ng-if='(videoStreams | numkeys).length == 0 && !captured'><span class='fa fa-info-circle'>You can use your device camera to take selfie photograph.</span></p><video id='video{{id}}' name='{{name}}' width='240' height='200' autoplay data-ng-show = '(videoStreams | numkeys).length > 0 && !captured'></video>" +
          "<canvas id='canvas{{id}}' width='240' height='200' data-ng-show='captured'></canvas></div><div class='col-xs-12 text-center'>" +
          "<button class='btn btn-primary' ng-click='captureCamera()' type='button' data-ng-show='!captured && (videoStreams | numkeys).length > 0'>Capture</button>" +
          "<button type='button' ng-click='tryagain()' class='btn btn-primary' data-ng-show='captured'>Try Again</button>" +
          "<button type='button' ng-click='upload()' data-ng-show='captured' class='btn btn-primary'>Upload</button>" +
          "<p class='err' data-ng-bind='err'></p></div>";
        return output;
      },
...
myApp.directive("customSelect", [
  "ajax",
  "$http",
  "$compile",
  function (ajax, $http, $compile) {
    return {
      restrict: "E",
      scope: {
        id: "@",
      ...
      template:
        '<select data-ng-model="selectmodel" class="form-control" id={{id}} name={{name}} data-ng-options="data.c as data.n for data in selectedItemList" data-ng-change="sendData(this.selectmodel)" data-ng-if="singleLocalSel" ></select>' +
      ...
      link: function (scope, element, attrs) {
        ...
        if (attrs.dptype == "single") {
          if (attrs.optntype == "local") {
            ...
            ajax.get(attrs.optRemoteUrl).then(
              function (response) {
...
myApp.directive("fileModel", [
  ...
  "$timeout",
  "cache",
  function ($q, ajax, $parse, $http, $compile, utilFunctions, $timeout, cache) {
    function getDocSize() {
      if (cache.get("docsize").length > 0) {
        return cache.get("docsize");
      }
      var deferred = $q.defer();
      ajax.get("/master/docsize").then(
        function (resp) {
    ...
    function link(scope, element, attrs) {
      ...
      scope.validateFile = function () {
        ...
        if (file.name && file.name.length > 75) {
          scope.setError(
            "Invalid filename, filename can not exceed 75 characters"
          );
          return false;
        }
        ...
        if (file.size / 1024 <= scope.allow_size) {
          ...
          if (scope.allow_size >= 1000) {
            ...
            scope.setError(
              "Max file upload size is " + scope.allow_size + " KB"
            );
          }
        }
      };
    ...
    return {
      ...
      template: function (attrs) {
        var output =
          "<input id='{{id}}' name='{{name}}' type='file' '{{required}}'";
        output +=
          "/><p class='err' data-ng-bind='errVar'></p><div class='progress' data-ng-show='uploading'>" +
...
myApp.directive("confClick", [
  "utilFunctions",
  "$compile",
  function (utilFunctions, $compile) {
    return {
      priority: -1,
      restrict: "A",
      scope: {
              ...
              if (playPromise !== undefined) {
                playPromise.then(
                  ...
                  function (error) {
                    deferred.reject({
                      error:
                        "Error in playback. Unable to get media from device camera.",
                    });
                  }
                );
              }
          ...
          .each(function () {
            if (
              ...
              if (scope.$parent[attrs["name"]][$(this).attr("name")]) {
                scope.$parent[attrs["name"]][$(this).attr("name")].$setValidity(
                  "text",
                  false
                );
              }
...
myApp.directive("autocompleteDropdown", [
  "$q",
  "$parse",
  "$http",
  "$sce",
  ...
  function (
    ...
    function link(scope, elem, attrs, ctrl) {
      ...
      function dropdownRow() {
        return elem[0].querySelectorAll(".autocomplete-row")[
          scope.currentIndex
        ];
      }
      ...
      function dropdownRowTop() {
        return (
          dropdownRow().getBoundingClientRect().top -
          (dd.getBoundingClientRect().top +
      ...
      scope.selectResult = function (result) {
        ...
        if (
          scope.clearResultAfterSelect === "true" ||
          scope.clearResultAfterSelect === true
        ) {
          scope.searchStr = "";
        } else {
...
myApp.directive("attachEvc", [
  "ajax",
  "$compile",
  "$timeout",
  function (ajax, $compile, $timeout) {
...
myApp.directive("attachEsign", [
  ...
  "$location",
  "$timeout",
  function (ajax, utilFunctions, $compile, shareData, $location, $timeout) {
            ...
            function (response) {
              ...
              if (response.stscd == "1" || response.status == "1") {
                ...
                if (response.errorDetail)
                  scope.setErr(response.errorDesc, "error");
              }
            },
            function () {
              $().blockPage(false);
              ...
              if (response.stscd == "0" || response.status == "0") {
                ...
                if (response.errorDetail) {
                  if (response.errorDetail.errorCd == "K-100") {
                    scope.setErr(
                      "Invalid OTP. Kindly enter valid OTP.",
                      "error"
                    );
                  } else {
                    scope.setErr(
      ...
      scope.callDSC = function () {
        ...
        scope.dscAction = function (dhash) {
          ...
          scope.connection.onmessage = function (resp) {
            if (
              resp.data === "signing canceled" ||
              resp.data === "signing failed"
            ) {
              $().blockPage(false);
              ...
              if (
                resp.data.substring(11, resp.data.indexOf("SerialNo") - 1) != ""
              ) {
                var params = {
                  hContent: dhash,
                  emSignData: resp.data.substring(
                    11,
                    resp.data.indexOf("SerialNo") - 1
                  ),
                  pan: scope.panAuth,
                ...
                ajax
                  .post(scope.signUrl, params, {}, function () {
                ...
                scope.setError(
                  "Sorry, No valid digital signatures found. Please try again"
                );
              }
            }
          };
          ...
          .then(
            function (data) {
              if (data.stscd === "FAILURE") {
                scope.waitMessage =
                  "The DSC you are trying to affix is not registered on the portal. Please use another DSC which is already registered or register the new DSC and then affix it.";
                scope.disableEle = false;
                scope.resClass = "alert-danger";
                $().blockPage(false);
              } else {
                scope.waitMessage =
                  "Retrieving installed digital signatures.. ";
                scope.resClass = "alert-warning";
                ...
                hash.then(
                  function (dhash) {
                    if (!scope.connection) {
                      $().blockPage(true);
                      utilFunctions.connectEmsigner().then(
                        function (conn) {
                        ...
                        function (err) {
                          ...
                          if (err.status && err.status === "500") {
                            scope.waitMessage =
                              "Please install and use the correct EMSigner. The software can be downloaded from System Requirements page, link given in footer.";
                          } else {
                            scope.waitMessage =
          ...
          scope.$apply(function () {
            ...
            if (!isNaN(parseFloat(plainNumber).toFixed(attrs.fraction))) {
              if (value1 >= 1000000000000000 && attrs.id === "invval") {
                var value =
                  parseFloat(parseFloat(plainNumber).toFixed(attrs.fraction)) -
                  0.1;
                ctrl.$setViewValue(value);
              } else {
                ctrl.$setViewValue(
                  parseFloat(plainNumber).toFixed(attrs.fraction)
                );
              }
            } else {
              ctrl.$setViewValue("");
            ...
            if (
              !isNaN(parseFloat(plainNumber).toFixed(attrs.fraction)) &&
              attrs.format &&
              attrs.format.length > 0
            ) {
              if (value1 >= 1000000000000000 && attrs.id === "invval") {
                var value =
                  parseFloat(parseFloat(plainNumber).toFixed(attrs.fraction)) -
                  0.1;
                elem.val(
                  $filter(attrs.format)(value.toString(), "", attrs.fraction)
                ); // removed ₹ symbol as param for ICR 17052
              } else {
                elem.val(
                  $filter(attrs.format)(plainNumber, "", attrs.fraction)
                ); // removed ₹ symbol as param for ICR 17052
              }
            } else {
...
myApp.filter("words", function () {
  ...
  return function (value) {
    function toWords(a) {
      ...
      c =
        " One Two Three Four Five Six Seven Eight Nine Ten Eleven Twelve Thirteen Fourteen Fifteen Sixteen Seventeen Eightteen Nineteen".split(
          " "
        );
...
myApp.directive("captcha", function () {
  function link(scope, element, attrs) {
    ...
    scope.loadCaptcha = function () {
      ...
      $("#imgCaptcha").on("error", function () {
        ...
        scope.setResetErr(
          "captchaErr",
          "Unable to load CAPTCHA image. Please click refresh button to load."
        );
      });
    };
    scope.loadCaptcha();
  ...
  return {
    ...
    template: function (ele, attrs, scope) {
      return (
        '<audio id="audioCap" ng-if="playAudio" autoplay></audio>' +
...
myApp.factory("FormSubmitter", [
  "$rootScope",
  "$sce",
  "$compile",
  function ($rootScope, $sce, $compile) {
    return {
      submit: function (url, method, params) {
        $("body").append(
          $compile(
            "<data-form-submitter-directive></data-form-submitter-directive>"
          )($rootScope)
        );
...
myApp.directive("formSubmitterDirective", [
  "$timeout",
  "$rootScope",
  function ($timeout, $rootScope) {
    return {
      ...
      replace: true,
      template:
        '<form id="formm" action="{{ formData.url }}" method="{{ formData.method }}">' +
...
myApp.directive("percentage", function () {
  var link = function (scope, element, attrs, ngModel) {
    ...
    element.bind("blur", function () {
      if (
        !isNaN(ngModel.$modelValue) &&
        parseFloat(ngModel.$modelValue) !== 0
      ) {
...
myApp.directive("hsnSac", function () {
  var link = function (scope, element, attrs, ngModel) {
    ngModel.$parsers.push(function (input) {
      ...
      cleanInputValue =
        cleanInputValue.length > 8
          ? cleanInputValue.slice(0, -1)
          : cleanInputValue;
...
myApp.directive("newAutocompleteHsn", [
  "$q",
  "$parse",
  "$http",
  "$sce",
  ...
  function (
    ...
    function link(scope, elem, attrs, ctrl) {
      ...
      function processResults(responseData, str) {
        var i, description, text, formattedText, formattedDesc;
        responseData = filterResultByCodeLength(
          responseData,
          scope.minCodeLengthToDisplay
        );
        if (responseData && responseData.length > 0) {
          scope.results = [];
          for (i = 0; i < responseData.length; i = i + 1) {
      ...
      function keyupHandler(event) {
        ...
        if (which === KEY_UP || which === KEY_EN) {
          ...
          if (
            ...
            !pattern.test(scope.searchStr) &&
            scope.searchStr.length >= Math.max(4, parseInt(minlength))
          ) {
            initResults();
            if (searchTimer) {
              $timeout.cancel(searchTimer);
          ...
          hideTimer = $timeout(function () {
            ...
            scope.$apply(function () {
              if (scope.searchStr && scope.searchStr.length > 0) {
                ...
                const result = altFind(scope.results, function (e) {
                  return e.description === scope.searchStr;
                });
      ...
      scope.inputChangeHandler = function (str) {
        ...
        if (str && str.length < minlength) {
          ...
          str &&
          !pattern.test(str) &&
          str.length < Math.max(4, minlength)
        ) {
...
myApp.directive("mstrAutocompleteHsn", [
  "$q",
  "$parse",
  "$http",
  "$sce",
    ...
    function link(scope, elem, attrs, ctrl) {
      ...
      function keyupHandler(event) {
        ...
        if (which === KEY_UP || which === KEY_EN) {
          ...
          if (
            ...
            scope.searchStr.length >= minlength &&
            pattern.test(scope.searchStr) &&
            (scope.searchStr.length == 4 ||
              scope.searchStr.length == 6 ||
              scope.searchStr.length == 8)
          ) {
            scope.searching = true;
            initResults();
            searchTimerComplete(scope.searchStr);
      ...
      scope.inputChangeHandler = function (str) {
        ...
        if (str && str.length < minlength) {
          ...
          str &&
          pattern.test(str) &&
          (str.length != 4 || str.length != 6 || str.length != 8)
        ) {
          clearResults();
          scope.searching = false;
          showAll();
      ...
      link: function (scope, elem, attrs, ctrl) {
        ...
        ctrl.$formatters.unshift(function (a) {
          ...
          if (attrs.formatting && attrs.formatting.length > 0) {
            if (attrs.name !== "qty")
              return $filter(attrs.formatting)(
                ctrl.$modelValue,
                "",
                attrs.fraction
              ); // removed ₹ symbol as param for ICR 17052
          ...
          scope.$apply(function () {
            ...
            if (
              ...
              if (value1 >= 1000000000000000 && attrs.id === "invval") {
                ...
                elem.val(
                  $filter(attrs.formatting)(plainNumber, "", attrs.fraction)
                ); // removed ₹ symbol as param for ICR 17052
              }
            } else {
      ...
      scope.validateFile = function () {
        ...
        if (file.size / 1024 <= scope.allow_size) {
          ...
          if (scope.allow_size >= 8192) {
            ...
            scope.setError(
              "Max file upload size is " + scope.allow_size / 1024 + " MB"
            );
          }
        }
      };
...
myApp.factory("Auth", function () {
  ...
  return {
    setUser: function (aUser) {
      sessionStorage.setItem("user", aUser);
    },
    getUser: function () {
