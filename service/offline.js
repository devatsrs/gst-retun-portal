      ...
      switch (key) {
        ...
        case "b2cla":
          for (var i = 0; i < file[key].length; i++) {
            for (var j = 0; j < file[key][i].inv.length; j++) {
              var invoice = file[key][i].inv[j];
              if (invoice.inv_typ) delete invoice.inv_typ;
            }
          }
          break;
        case "exp":
        case "expa":
          for (var i = 0; i < file[key].length; i++) {
            for (var j = 0; j < file[key][i].inv.length; j++) {
              var invoice = file[key][i].inv[j];
              if (invoice.diff_percent) delete invoice.diff_percent;
            }
          }
          break;
        default:
...
function saveMstrforprod(req, res, err) {
  ...
  try {
    var fileData = fs.readFileSync(
      __dirname + "/../public/userData/master_" + gstin + ".json",
      "utf8"
    );
    fileData = JSON.parse(fileData);
    var pMaster =
      rectype == "productsMasters" ? mstrData : fileData.productsMasters;
    var sMaster =
      rectype == "supplierRecipientMasters"
        ? mstrData
        : fileData.supplierRecipientMasters;
      ...
      for (var k = 0; k < to_be_checked.length; k++) {
        ...
        for (var i = 0; i < ctins; i++) {
          ...
          for (var j = 0; j < invs; j++) {
            if (tmp[j][key].toLowerCase() == val.toLowerCase()) return "yes";
          }
        }
      }
...
function itemExists(req, res) {
  ...
  if (!file_name) {
    ...
    var filename =
      dir + "/" + form + "_" + gstin + "_" + fy + "_" + month + ".json";
  } else {
  ...
  if (!sec || to_be_checked.length < 0) {
    res
      .status(200)
      .send({ result: "no", reason: "all_parameters_not_provided" });
    return;
  }
...
function listJsonData(req, res) {
  ...
  try {
    ...
    if (formName == "GSTR2" && sec == "nil") sec = "nil_supplies";
    if (sec == "atadj") {
      var sec2 = "txpd";
    } else if (sec == "cdnr") {
      var sec2 = "cdn";
    ...
    if (
      typeof fileData[sec] == "object" ||
      (sec2 != "" && typeof fileData[sec2] == "object")
    ) {
      ...
      var reformateInv = ReturnStructure.reformateInv(
        ReturnStructure.scopelists.suplyList,
        gstin,
        sec,
        formName,
        false,
      ...
      var formateNodePayload = ReturnStructure.formateNodePayload(
        sec,
        formName,
        shareData
      );
  ...
  try {
    ...
    update_meta_file(payloadfileName, payload, form, function (err) {
      if (err) {
        ...
        logger.log(
          "error",
          "Unexpected Error whilecreating summary :: %s",
          err.message
        );
    ...
    logger.log(
      "error",
      "Unexpected Error while generating file :: %s",
      err.message
    );
    response.error(errorObject, res);
  } finally {
...
function fixUQC(file_data) {
  ...
  for (var i = 0; i < data_length; i++) {
    file_data.hsn.data[i].uqc =
      typeof file_data.hsn.data[i].uqc == "string" &&
      file_data.hsn.data[i].uqc != ""
        ? file_data.hsn.data[i].uqc.split("-")[0].trim()
        : file_data.hsn.data[i].uqc;
  }
  return file_data;
}
...
function removePOS(data, type) {
  ...
  if (type == "cdnr" || type == "cdnra") {
    ...
    for (var i = 0; i < cdnrLen; i++) {
      ...
      if (data[i].nt && noteLen) {
        for (var j = 0; j < noteLen; j++) {
          ...
          if (data[i].nt[j].p_gst) delete data[i].nt[j].p_gst;

          if (data[i].nt[j].inum) delete data[i].nt[j].inum;

          if (data[i].nt[j].idt) delete data[i].nt[j].idt;

          if (data[i].nt[j].d_flag) delete data[i].nt[j].d_flag;

          if (data[i].nt[j].updby) delete data[i].nt[j].updby;

          if (data[i].nt[j].cflag) delete data[i].nt[j].cflag;

          if (data[i].nt[j].opd) delete data[i].nt[j].opd;

          if (data[i].nt[j].chksum) delete data[i].nt[j].chksum;
    ...
    for (var j = 0; j < cdnurLen; j++) {
      //changes for CDN delinking start
      if (data[j].p_gst) delete data[j].p_gst;

      if (data[j].inum) delete data[j].inum;

      if (data[j].idt) delete data[j].idt;

      if (data[j].d_flag) delete data[j].d_flag;

      if (data[j].updby) delete data[j].updby;
      //changes for CDN delinking end
      if (data[j].old_ntnum) delete data[j].old_ntnum;
      if (data[j].old_inum) delete data[j].old_inum;
    }
    return data;
  }
  return data;
...
function fixOldNum(data, type) {
  if (!data || (type != "hsn" && !data.length)) return data;
  ...
  if (
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i].old_inum) delete data[i].old_inum;
      if (data[i].old_ntnum) delete data[i].old_ntnum;
    }
  } else if (type == "hsn" && data.data) {
  ...
  if (
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i].inv && data[i].inv.length) {
        for (var j = 0; j < data[i].inv.length; j++) {
          ...
          if (
            (type == "b2cla" || type == "b2cl") &&
            data[i].inv[j].inv_typ != "CBW"
          )
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i].diff_percent) {
        ...
        if (data[i].diff_percent != 0.65) delete data[i].diff_percent;
      }
      delete data[i].inv_typ;
    }
  } else if (type == "cdnra" || type == "cdnr") {
...
function generateTotal(total, itmInner, iSec, InnerInv, typ) {
  ...
  if (
    typ == "SEWOP" &&
    (iSec == "b2b" || iSec == "b2ba" || iSec == "cdnr" || iSec == "cdnra")
  ) {
    itmInner.iamt = 0;
    itmInner.csamt = 0;
...
function generateItcTotal(total, itcInner, flag) {
  if (flag == "A" || flag == "M" || !flag) {
    total.itc_cgTl += itcInner.tx_c
      ? parseFloat(parseFloat(itcInner.tx_c).toFixed(2))
      : 0;
    total.itc_sgTl += itcInner.tx_s
      ? parseFloat(parseFloat(itcInner.tx_s).toFixed(2))
      : 0;
    total.itc_igTl += itcInner.tx_i
      ? parseFloat(parseFloat(itcInner.tx_i).toFixed(2))
      : 0;
    total.itc_csTl += itcInner.tx_cs
      ? parseFloat(parseFloat(itcInner.tx_cs).toFixed(2))
      : 0;
  }
  return total;
}
...
function reformSummary(iResp, prevContent, formName, callback) {
  ...
  for (var a = 0, alen = iResp.length; a < alen; a++) {
    var section = iResp[a];
    if (section.cd == "hsn" && formName == "GSTR2") section.cd = "hsnsum";
    var count = 0,
    ...
    if (
      section.cd == "atadj" &&
      ctinInv === undefined &&
      typeof prevContent["txpd"] !== "undefined"
    ) {
    ...
    if (
      section.cd == "atadja" &&
      ctinInv === undefined &&
      typeof prevContent["txpda"] !== "undefined"
    ) {
    ...
    if (
      section.cd == "cdnr" &&
      ctinInv === undefined &&
      typeof prevContent["cdn"] !== "undefined"
    ) {
    ...
    if (
      section.cd == "cdnra" &&
      ctinInv === undefined &&
      typeof prevContent["cdna"] !== "undefined"
    ) {
...
var update_meta_file = function (filename, json, form, cb) {
  ...
  var response = fs.readFileSync(
    __dirname + "/../public/data/tablename.json",
    "utf8"
  );
  ...
  reformSummary(response, json, formName, function (iData) {
    ...
    if (!cb) fs.writeFileSync(meta_filename, JSON.stringify(metaJSON));
    else {
...
var addtbldata = function (req, res) {
  ...
  try {
    ...
    async.waterfall(
      [
        ...
        function (formDataFormat, callback) {
          ...
          if (!fs.existsSync(dir)) {
            logger.log(
              "error",
              "error while creating the directory :: %s ",
              err.message
            );
          ...
          switch (tblcd) {
            ...
            case "b2ba":
              ...
              for (var i = 0; i < tbl_data.length; i++) {
                if (
                  gstin == tbl_data[i].ctin ||
                  gstin == tbl_data[i].inv[0].etin
                ) {
                  isSameGSTIN = gstin;
                } else {
            ...
            case "doc_issue":
              ...
              var keyObj = {};
              if (!gstfile.doc_issue) gstfile.doc_issue = { doc_det: [] };
              ...
              for (var i = 0; i < tbl_data.length; i++) {
                ...
                if (arrayFound.length != 0) {
                  // if doc_num already exists
                  var docAry = {};
                  docAry = arrayFound[0].docs;
                  for (
                    var inputRow = 0;
                    inputRow < tbl_data[0].docs.length;
                    inputRow++
                  ) {
                    ...
                    if (tbldata[j].pos === tbl_data[i].pos) {
                      if (tbldata[j].rt === tbl_data[i].rt) {
                        if (tbldata[j].etin === tbl_data[i].etin) {
                          if (
                            tbldata[j].diff_percent === tbl_data[i].diff_percent
                          ) {
                            tbldata.splice(j, 1);
            ...
            case "at":
              ...
              if (form == "GSTR1")
                for (var j = 0; j < tbldata.length; j++) {
                  if (!tbldata[j].diff_percent) tbldata[j].diff_percent = null;
                }
            ...
            case "hsn":
              ...
              for (var i = 0; i < tbl_data.length; i++) {
                ...
                for (var j = 0; j < tbldata.data.length; j++) {
                  var isReturnPeriodBeforeNewHSN =
                    isCurrentPeriodBeforeAATOCheck(newHSNStartDateConstant, fp);
                  if (isReturnPeriodBeforeNewHSN) {
                    if (tbldata.data[j].hsn_sc === tbl_data[i].data[0].hsn_sc) {
                      if (!tbldata.data[j].desc) tbldata.data[j].desc = "";
                      if (!tbl_data[i].data[0].desc)
                        tbl_data[i].data[0].desc = "";

                      if (
                        tbldata.data[j].desc.toLowerCase() ===
                        tbl_data[i].data[0].desc.toLowerCase()
                      ) {
                        if (
                          tbldata.data[j].uqc.toLowerCase() ===
                          tbl_data[i].data[0].uqc.toLowerCase()
                        ) {
                          if (
                            !isCurrentPeriodBeforeAATOCheck(
                              newHSNStartDateConstant,
                              fp
                            )
                          ) {
                            if (
                              tbldata.data[j].rt == tbl_data.dt[i].data[0].rt
                            ) {
                ...
                if (count != 1) {
                  ...
                  tbl_data[i].data[0].num =
                    total_hsn_objects >= maxNum ? total_hsn_objects : maxNum;
                  total_hsn_objects++;
            ...
            case "b2bura": //for GSTR2
              ...
              for (
                var i = 0;
                i < tbldata.length;
                i++ //loop to find all the existing invoice no.
              ) {
              ...
              for (
                var i = 0;
                i < tbl_data.length;
                i++ //loop to find all the previous invoice no.
              ) {
              ...
              for (
                var i = 0;
                i < responseinvce.length;
                i++ //loop to check that invoice no. exist or not
              ) {
                var count = 0;
            ...
            case "itc_rvsl": //for  GSTR2 31
              ...
              if (
                typeof tbl_data.length !== "undefined" &&
                tbl_data.length > 0
              ) {
                tbl_data = tbl_data[0];
              }
              ...
              for (var i = 0; i < tbl_data.length; i++) {
                ...
                for (var j = 0; j < tbldata.det.length; j++) {
                  if (tbldata.det[j].hsn_sc === tbl_data[i].hsn_sc) {
                    if (!tbldata.det[j].desc) tbldata.det[j].desc = "";
                    if (!tbl_data[i].desc) tbl_data[i].desc = "";

                    if (
                      tbldata.det[j].desc.toLowerCase() ===
                      tbl_data[i].desc.toLowerCase()
                    ) {
                      if (
                        tbldata.det[j].uqc.toLowerCase() ===
                        tbl_data[i].uqc.toLowerCase()
                      ) {
          ...
          if (jsonObj.length == 0) {
            logger.log(
              "info",
              "No duplicate invoice found and data added successfully"
            );
            ...
            logger.log(
              "info",
              "duplicate invoice found and non duplicated rows added successfully"
            );
      ...
      function (err, result) {
        ...
        if (err) {
          ...
          logger.log(
            "error",
            "Error While writing into the files :: %s",
            errorObject
          );
          response.error(errorObject, res);
        } else {
    ...
    logger.log(
      "error",
      "Unexpected Error while writing into the file :: %s",
      err
    );
    response.error(errorObject, res);
  } finally {
    ...
    fs.readFile(filename, "utf8", function (err, data) {
      if (err) console.log(err);
        ...
        if (type == "Import") {
          ...
          async.waterfall(
            [
              function (callback) {
                ...
                common.formDataFormat(req, function (formDataFormat) {
                  logger.log(
                    "info",
                    "entered in async.waterfall formDataFormat"
                  );
                  callback(null, formDataFormat);
                });
              },
              function (formDataFormat, callback) {
                ...
                logger.log(
                  "info",
                  " creating formDataFormat of form :: %s ",
                  form
                );
                if (!fs.existsSync(dir)) {
                  // when user will come first time and no directory is there to save the file.
                  // user will enter only once in this.
                  //After entering  file will get created.
                  mkdirp(
                    dir,
                    function (
                      err // if directory is not there will create directory
                    ) {
                      if (err) {
                        // if we are facing issue in creating the directory
                        logger.log(
                          "error",
                          "Unexpected error while creating the directory:: %s",
                          err.message
                        );
                        callback(err, null);
                      } // if we are not facing issue in creating the directory.
                      else {
                        fs.writeFile(
                          filename,
                          formDataFormat,
                          function (
                            err // after creating the directory we are creating file inside that in order to save the table data.
                          ) {
                            if (err) {
                              // if we are facing issue in creating the file
                              logger.log(
                                "error",
                                "Unexpected error while creating the file:: %s",
                                err.message
                              );
                              callback(err, null);
                            } // file is created
                              ...
                              fs.readFile(
                                ...
                                function (err, data) {
                                  if (err) {
                                    //if we are unable to read the file
                                    logger.log(
                                      "error",
                                      "Unexpected error while reading the file:: %s",
                                      err.message
                                    );
                                    callback(err, null);
                                  } // if we are able to read the file
                                  else {
              ...
              function (gstfileNew, callback) {
                  ...
                  for (var key in gstfile) {
                    ...
                    switch (key) {
                      ...
                      case "b2ba":
                        ...
                        for (var i = 0; i < tmp_1; i++) {
                          ...
                          for (var j = 0; j < tmp_2; j++) {
                            ...
                            if (
                              !value[i].inv[j].hasOwnProperty("flag") ||
                              value[i].inv[j].flag === "D" ||
                              value[i].inv[j].flag === "A" ||
                              value[i].inv[j].flag === "R" ||
                              value[i].inv[j].flag === "P" ||
                              value[i].inv[j].flag === "M" ||
                              value[i].inv[j].flag === "E" ||
                              value[i].inv[j].flag === ""
                            ) {
                              /** cloned object is created to delete the flag if flag is M or E*/
                              var clonedObject = extend(
                                true,
                                {},
                                value[i].inv[j]
                              );
                              ...
                              if (clonedObject.updby == "S") {
                                if (clonedObject.hasOwnProperty("chksum")) {
                                  if (
                                    value[i].inv[j].hasOwnProperty("isActed")
                                  ) {
                      ...
                      case "b2csa":
                        for (var i = value.length - 1; i >= 0; i--) {
                          if (
                            ...
                            value[i].flag === "D" ||
                            value[i].flag === "E" ||
                            !value[i].hasOwnProperty("flag")
                          ) {
                            delete value[i]["uni_key"];
                            var clonedObject = extend(true, {}, value[i]);
                            if (value[i].flag === "E") delete clonedObject.flag;
                            //Added by Subrat for 65% related CR
                            delete clonedObject["diffval"];
                        ...
                        for (var i = value.length - 1; i >= 0; i--) {
                          ...
                          for (var j = value[i].nt.length - 1; j >= 0; j--) {
                            if (
                              ...
                              value[i].nt[j].flag === "A" ||
                              value[i].nt[j].flag === "" ||
                              !value[i].nt[j].hasOwnProperty("flag")
                            ) {
                              ...
                              if (clonedObject.updby == "S") {
                                if (clonedObject.hasOwnProperty("chksum")) {
                                  if (
                                    value[i].nt[j].hasOwnProperty("isActed")
                                  ) {
                                    delete clonedObject["isActed"];
                                    delete clonedObject["updby"];
                                    newInv.push(clonedObject);
                          ...
                          for (var i = value.length - 1; i >= 0; i--) {
                            if (
                              ...
                              if (key == "txpd" || key == "atadj") {
                                if (!gstfileNew.atadj) gstfileNew.atadj = [];
                                if (!gstfileNew.txpd) gstfileNew.txpd = [];
                                gstfileNew.atadj.push(clonedObject);
                              } else {
                                if (!gstfileNew.atadja) gstfileNew.atadja = [];

                                if (!gstfileNew.txpda) gstfileNew.txpda = [];
                      ...
                      case "nil":
                        ...
                        if (clone.chksum) delete clone.chksum;
                      ...
                      case "itc_rvsl":
                        if (
                          value.flag === "E" ||
                          (value.flag === "M" && value.flag === "D") ||
                          !value.hasOwnProperty("flag")
                        ) {
                      ...
                      case "hsnsum":
                        ...
                        if (!value.det) value.det = [];
                        for (var j = value.det.length - 1; j >= 0; j--) {
                          if (
                            value.det[j].flag === "E" ||
                            value.det[j].flag === "M" ||
                            !value.det[j].hasOwnProperty("flag")
                          ) {
                ...
                if (byteSize < max_size) {
                  ...
                  var date_stamp =
                    ...
                    "s";
                  var fp_date =
                    sys_date.getDate().toString() +
                    (sys_date.getMonth() + 1).toString() +
                    sys_date.getFullYear().toString();
                    ...
                    function (err) {
                      ...
                      fs.access(
                        ...
                        function (exists) {
                          if (exists) {
                            // do stuff
                            respObj.down_dir = single_dir.replace(
                              "./public/",
                              ""
                            );
            ...
            function (err, result) {
              ...
              if (err) {
                ...
                logger.log(
                  "error",
                  "Error While adding the invoices :: %s",
                  errorObject
                );
                response.error(errorObject, res);
              } else {
                logger.log(
                  "info",
                  "Return Details Added Successfully :: %s",
                  result
                );
                response.success(result, res);
              }
          ...
          if (gstfile.b2csa) gstfile.b2csa = fixPOS(gstfile.b2csa, "b2csa");
...
var deleteallinvoices = function (req, res) {
  ...
  try {
    async.waterfall(
      ...
      function (err, result) {
        ...
        if (err) {
          ...
          logger.log(
            "error",
            "Error While deleting the files :: %s",
            errorObject
          );
          response.error(errorObject, res);
        } else {
    ...
    logger.log(
      "error",
      "Unexpected Error while deleting the file:: %s",
      err.message
    );
    response.error(errorObject, res);
  } finally {
...
var deleteMltplInv = function (req, res) {
  ...
  try {
      ...
      [
        function (callback) {
          ...
          if (type == "Import") {
            ...
            var dir =
              controlFiledir + gstin + "/" + form + "/" + fy + "/" + month;
            filename =
              dir + "/" + form + "_" + gstin + "_" + fy + "_" + month + ".json";
          }
              ...
              switch (tblcd) {
                ...
                case "b2ba":
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    if (arrayFound.length <= 1) {
                      ...
                      if (
                        arrayFound[0].inv.length == 0 ||
                        arrayFound[0].inv.length == 1
                      ) {
                        tbldata.splice(index, 1);
                      } else {
                        var subarray = {};
                ...
                case "cdnra":
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    if (arrayFound.length <= 1) {
                      var index = tbldata.indexOf(arrayFound[0]); // to find the index of the object
                      if (
                        arrayFound[0].nt.length == 0 ||
                        arrayFound[0].nt.length == 1
                      ) {
                        tbldata.splice(index, 1);
                      } else {
                        var subarray = {};
                ...
                case "txia":
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    if (arrayFound.length == 1) {
                      //no other case is possible.because delete will be called for existing pos only.So array found will always have a value of 1.
                      var index = tbldata.indexOf(arrayFound[0]);
                      tbldata.splice(index, 1);
                    }
                  }
                ...
                case "atadja":
                  ...
                  if (form == "GSTR1") {
                    ...
                    for (var i = 0; i < invdltArray.length; i++) {
                      ...
                      if (arrayFound.length == 1) {
                        //no other case is possible.because delete will be called for existing cpty only.So array found will always have a value of 1.
                        var index = tbldata.indexOf(arrayFound[0]);
                        tbldata.splice(index, 1);
                      }
                    }
                ...
                case "hsn":
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    if (arrayFound.length == 1) {
                      //no other case is possible.because delete will be called for existing num only.So array found will always have a value of 1.
                ...
                case "hsnsum": //for  GSTR2
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    if (arrayFound.length == 1) {
                      //no other case is possible.because update will be called for existing num only.So array found will always have a value of 1.
    ...
    logger.log(
      "error",
      "Unexpected error while deleting the invoices:: %s",
      err.message
    );
    response.error(errorObject, res);
  } finally {
    ...
    async.waterfall(
      ...
      function (err, result) {
        ...
        if (err) {
          ...
          logger.log(
            "error",
            "Error While uploading the files :: %s",
            errorObject
          );
          response.error(errorObject, res);
        } else {
            ...
            for (var key in gstfile) {
              ...
              switch (key) {
                ...
                case "hsn":
                  ...
                  if (tbldata["flag"]) {
                    //to check whether the payload has "flag" property - prakash
                  ...
                  for (var k = 0; k < tbldata.data.length; k++) {
                    ...
                    if (fistTbldata.data.length) {
                      for (var j = 0; j < fistTbldata.data.length; j++) {
                        if (
                          tbldata.data[k].hsn_sc === fistTbldata.data[j].hsn_sc
                        ) {
                          if (!tbldata.data[k].desc) tbldata.data[k].desc = "";
                          if (!fistTbldata.data[j].desc)
                            fistTbldata.data[j].desc = "";

                          if (
                            tbldata.data[k].desc.toLowerCase() ===
                            fistTbldata.data[j].desc.toLowerCase()
                          ) {
                            if (
                              tbldata.data[k].uqc.toLowerCase() ===
                              fistTbldata.data[j].uqc.toLowerCase()
                            ) {
                              if (
                                !isCurrentPeriodBeforeAATOCheck(
                                  newHSNStartDateConstant,
                                ...
                                if (
                                  tbldata.data[j].rt == secData.dt[i].data[0].rt
                                ) {
                                  count++;
                                }
                              } else {
                  ...
                  for (var k = 0; k < tbldata.det.length; k++) {
                    ...
                    if (fistTbldata.det.length) {
                      for (var j = 0; j < fistTbldata.det.length; j++) {
                        if (
                          tbldata.det[k].hsn_sc === fistTbldata.det[j].hsn_sc
                        ) {
                          if (!tbldata.det[k].desc) tbldata.det[k].desc = "";
                          if (!fistTbldata.det[j].desc)
                            fistTbldata.det[j].desc = "";

                          if (
                            tbldata.det[k].desc.toLowerCase() ===
                            fistTbldata.det[j].desc.toLowerCase()
                          ) {
                            if (
                              tbldata.det[k].uqc.toLowerCase() ===
                              fistTbldata.det[j].uqc.toLowerCase()
                            ) {
                              count++;
                            }
                          }
...
var iffflow = function (obj) {
  ...
  if (rtn_prd % 3 !== 0) {
    if (obj.hasOwnProperty("error_report")) {
      if (
        obj["error_report"].hasOwnProperty("b2cl") ||
        obj["error_report"].hasOwnProperty("b2cla") ||
        obj["error_report"].hasOwnProperty("b2cs") ||
        obj["error_report"].hasOwnProperty("b2csa") ||
        obj["error_report"].hasOwnProperty("cdnur") ||
        obj["error_report"].hasOwnProperty("cdnura") ||
        obj["error_report"].hasOwnProperty("exp") ||
        obj["error_report"].hasOwnProperty("expa") ||
        obj["error_report"].hasOwnProperty("at") ||
        obj["error_report"].hasOwnProperty("ata") ||
        obj["error_report"].hasOwnProperty("atadj") ||
        obj["error_report"].hasOwnProperty("atadja") ||
        obj["error_report"].hasOwnProperty("nil") ||
        obj["error_report"].hasOwnProperty("hsn") ||
        obj["error_report"].hasOwnProperty("doc_issue")
      ) {
        flagJson = true;
      }
    } else if (
      obj.hasOwnProperty("b2cl") ||
      obj.hasOwnProperty("b2cla") ||
      obj.hasOwnProperty("b2cs") ||
      obj.hasOwnProperty("b2csa") ||
      obj.hasOwnProperty("cdnur") ||
      obj.hasOwnProperty("cdnura") ||
      obj.hasOwnProperty("exp") ||
      obj.hasOwnProperty("expa") ||
      obj.hasOwnProperty("at") ||
      obj.hasOwnProperty("ata") ||
      obj.hasOwnProperty("atadj") ||
      obj.hasOwnProperty("atadja") ||
      obj.hasOwnProperty("nil") ||
      obj.hasOwnProperty("hsn") ||
      obj.hasOwnProperty("doc_issue")
    ) {
      flagJson = true;
    }
  }
    ...
    async.waterfall(
      [
        function (callback) {
          ...
          callback(
            null,
            "error file unzipped successfully",
            "./public/error/" + folder_name
          );
        },
        function (msg, fname, callback) {
          ...
          readFiles(
            fname,
            function (filename, content, flag) {
      ...
      function (err, result) {
        ...
        if (err) {
          ...
          logger.log(
            "error",
            "Error While unzipping the error zip:: %s",
            errorObject
          );
          response.error(errorObject, res);
        } else {
          logger.log(
            "info",
            "Unzipping successful for the error zip :: %s",
            result
          );
          response.success(result, res);
        }
    ...
    async.waterfall(
      ...
      function (err, result) {
        ...
        if (err) {
          ...
          logger.log(
            "error",
            "Error While unzipping the zip:: %s",
            errorObject
          );
          response.error(errorObject, res);
        } else {
...
var updatetbldata = function (req, res) {
  ...
  try {
    async.waterfall(
      [
        function (callback) {
          ...
          logger.log(
            "info",
            "Entering Offline File:: updatetbldata with tbl_data :: %s",
            tbl_data
          );
          logger.log(
            "info",
            "Entering Offline File:: updatetbldata with tblcd :: %s",
            tblcd
          );
          ...
          if (type == "Upload") {
            ...
            filename =
              dir + "/" + gstin + "_" + form + "_" + fy + "_" + month + ".json";
          } else if (type == "Error") {
              ...
              switch (tblcd) {
                ...
                case "doc_issue":
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    if (arrayFound.length != 0) {
                      ...
                      for (
                        var inputRow = 0;
                        inputRow < invdltArray[0].docs.length;
                        inputRow++
                      ) {
                        var subArrayFound = docAry.myFind({
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    if (arrayFound.length <= 1) {
                      ...
                      if (
                        ...
                        if (
                          subArrayFound2.length > 0 &&
                          invdltArray[i].old_ntnum != "" &&
                          invdltArray[i].nt_num != invdltArray[i].old_ntnum
                        ) {
                ...
                case "cdnura":
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    if (arrayFound.length == 1) {
                      //no other case is possible.because update will be called for existing nt_num only.So array found will always have a value of 1.
                      var index = tbldata.indexOf(arrayFound[0]);
                ...
                case "at":
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    if (arrayFound.length == 1) {
                      //no other case is possible.because update will be called for existing pos only.So array found will always have a value of 1.
                      var index = tbldata.indexOf(arrayFound[0]);
                      tbldata.splice(index, 1); //delete row first with matched doc_num
                      tbldata.splice(index, 0, tbl_data[i]); //insert updated row in the same index of previous row
                    }
                ...
                case "atadj":
                  ...
                  if (form == "GSTR1") {
                    ...
                    for (var i = 0; i < invdltArray.length; i++) {
                      ...
                      if (arrayFound.length == 1) {
                        //no other case is possible.because update will be called for existing cpty only.So array found will always have a value of 1.
                        var index = tbldata.indexOf(arrayFound[0]);
      ...
      function (err, result) {
        ...
        if (err) {
          ...
          logger.log(
            "info",
            "Error While updating the documents :: %s",
            errorObject
          );
          response.error(errorObject, res);
        } else {
    ...
    logger.log(
      "error",
      "Unexpected error while updating the data:: %s",
      err.message
    );
    response.error(errorObject, res);
  } finally {
    ...
    async.waterfall(
      [
        function (callback) {
          ...
          logger.log(
            "info",
            "Entering Offline File:: updateerrdata with tbl_data :: %s",
            tbl_data
          );
          logger.log(
            "info",
            "Entering Offline File:: updateerrdata with tblcd :: %s",
            tblcd
          );
          var invdltArray = req.body.invdltArray; // this will contain an array of objects.Each object will consist of ctin and respective invoice no. to update
          var type = req.body.type;
            ...
            if (err) {
              ...
              switch (tblcd) {
                ...
                case "b2ba":
                  ...
                  logger.log(
                    "info",
                    "Entering Offline File:: updateerrdata with b2b"
                  );
                ...
                case "b2cla":
                  ...
                  logger.log(
                    "info",
                    "Entering Offline File:: updateerrdata with b2cl"
                  );
                      ...
                      tbldata.forEach(function (row, outerIndex) {
                        row.data.forEach(function (data, innerIndex) {
                          if (data.num == invdltArray[0].num) {
                            tbldata[outerIndex].data[innerIndex] =
                              tbl_data[0].data[0];
                          }
                        });
                      });
      ...
      function (err, result) {
        ...
        if (err) {
          ...
          logger.log(
            "error",
            "Error While updating the invoices :: %s",
            errorObject
          );
          response.error(errorObject, res);
        } else {
          logger.log(
            "info",
            "Return document updated successfully:: %s",
            result
          );
          response.success(result, res);
        }
            ...
            if (err) {
              ...
              switch (tblcd) {
                  ...
                  if (form == "GSTR1") {
                      ...
                      if (arrayFound.length <= 1) {
                        ...
                        if (
                          (tbl_data[i].inv[0].flag == "M" ||
                            tbl_data[i].inv[0].flag == "N") &&
                          typeof backups[tblcd][invdltArray[i].inum] ===
                            "undefined"
                        ) {
                          if (
                            tbl_data[0].inv[0].idt == subarray[subIndex].idt &&
                            tbl_data[0].inv[0].val == subarray[subIndex].val &&
                            tbl_data[0].inv[0].rchrg ==
                              subarray[subIndex].rchrg &&
                            tbl_data[0].inv[0].pos == subarray[subIndex].pos &&
                            tbl_data[0].inv[0].inv_typ ==
                              subarray[subIndex].inv_typ
                          ) {
                            // main level is same
                            if (
                              subarray[subIndex].itms.length ==
                              tbl_data[0].inv[0].itms.length
                            ) {
                              for (
                                var k = 0;
                                k < subarray[subIndex].itms.length;
                                k++
                              ) {
                                if (
                                  _.isEmpty(
                                    common.diffObject(
                                      subarray[subIndex].itms[k].itm_det,
                                      tbl_data[0].inv[0].itms[k].itm_det
                                    )
                                  )
                                ) {
                                  if (
                                    _.isEmpty(
                                      common.diffObject(
                                        subarray[subIndex].itms[k].itc,
                                        tbl_data[0].inv[0].itms[k].itc
                                      )
                                    )
                                  ) {
                                    // no change found
                          ...
                          if (backups[tblcd][invdltArray[i].inum] === undefined)
                            backups[tblcd][invdltArray[i].inum] =
                              subarray[subIndex];
                        }
                    ...
                    for (var i = 0; i < invdltArray.length; i++) {
                      ...
                      if (arrayFound.length <= 1) {
                        ...
                        var subIndex = subarray.indexOf(subArrayFound[0]);
                        if (
                          typeof tbl_data[i].inv !== "undefined" &&
                          typeof tbl_data[i].inv[0] !== "undefined"
                        ) {
                        ...
                        if (
                          ...
                          if (
                            ...
                            tbl_data[i].rchrg == subarray[subIndex].rchrg
                          ) {
                            // main level is same

                            if (
                              subarray[subIndex].itms.length ==
                              tbl_data[i].itms.length
                            ) {
                              for (
                                var k = 0;
                                k < subarray[subIndex].itms.length;
                  ...
                  if (form == "GSTR1") {
                      ...
                      if (arrayFound.length <= 1) {
                        ...
                        if (
                          (tbl_data[i].nt[0].flag == "M" ||
                            tbl_data[i].nt[0].flag == "N") &&
                          typeof backups[tblcd][invdltArray[i].nt_num] ===
                            "undefined"
                        ) {
                          if (
                            tbl_data[0].nt[0].ntty == subarray[subIndex].ntty &&
                            tbl_data[0].nt[0].nt_num ==
                              subarray[subIndex].nt_num &&
                            tbl_data[0].nt[0].nt_dt ==
                              subarray[subIndex].nt_dt &&
                              ...
                              for (
                                ...
                                if (
                                  _.isEmpty(
                                    common.diffObject(
                                      subarray[subIndex].itms[k].itm_det,
                                      tbl_data[0].nt[0].itms[k].itm_det
                                    )
                                  )
                                ) {
                                  if (
                          ...
                          if (
                            backups[tblcd][invdltArray[i].nt_num] === undefined
                          )
                            backups[tblcd][invdltArray[i].nt_num] =
                              subarray[subIndex];
                        }
                    ...
                    for (var i = 0; i < invdltArray.length; i++) {
                      ...
                      if (arrayFound.length <= 1) {
                        ...
                        if (
                          typeof tbl_data[i].nt !== "undefined" &&
                          typeof tbl_data[i].nt[0] !== "undefined"
                        ) {
                ...
                default:
                  logger.log(
                    "error",
                    "This functionality is only for b2b and cdnr section and getting ::%s",
                    tblcd
                  );
              }
            }
      ...
      function (err, result) {
        ...
        if (err) {
          ...
          if (err == "200OK") {
            logger.log(
              "error",
              "User cannot modify the rejected invoices :: %s",
              err
            );
            response.success(
              "You cannot modify the invoice if you are rejecting the invoice",
              res
            );
          } else {
            errorObject = {
...
var addmltpldata = function (req, res) {
  ...
  try {
    ...
    async.waterfall(
      [
        function (callback) {
          logger.log(
            "info",
            "entered in async.waterfall function 0, cehcking for data-cache"
          );
            ...
            myCache.get(dataObj.cache_key, function (err, value) {
              if (!err) {
                if (value == undefined) {
                  logger.log(
                    "warning",
                    "cache key found, but not the data, something wrong, abort"
                  );
                ...
                logger.log(
                  "warning",
                  "cache key found, but unable to get the data, something wrong, abort"
                );
                callback(err, null);
              }
            });
        ...
        function (formDataFormat, callback) {
                          ...
                          if (err) {
                            ...
                            for (var k = 0; k < dataObj.length; k++) {
                              ...
                              if (!secData.dt[0]) {
                                logger.log(
                                  "info",
                                  "data is not there so no need to add empty array"
                                );
                              } else {
                                ...
                                switch (secData.cd) {
                                  ...
                                  case "b2ba":
                                    ...
                                    for (
                                      ...
                                      if (
                                        gstin == secData.dt[p].ctin ||
                                        gstin == secData.dt[p].inv[0].etin
                                      ) {
                                        isSameGSTIN = gstin;
                                      } else {
                                        var arrayFound = tbldata.myFind({
                                  ...
                                  case "doc_issue":
                                    ...
                                    for (
                                      ...
                                      if (arrayFound.length != 0) {
                                        ...
                                        for (
                                          var inputRow = 0;
                                          inputRow < secData.dt[i].docs.length;
                                          inputRow++
                                        ) {
                                          var subArrayFound = docAry.myFind({
                                            num: secData.dt[i].docs[inputRow]
                                              .num,
                                          });

                                          if (subArrayFound.length != 0) {
                                            var subIndex = docAry.indexOf(
                                              subArrayFound[0]
                                            );
                                            docAry.splice(subIndex, 1);
                                            docAry.splice(
                                    ...
                                    for (
                                      ...
                                      if (gstin == secData.dt[i].etin) {
                                        ...
                                        for (
                                          ...
                                          if (
                                            tbldata[j].pos === secData.dt[i].pos
                                          ) {
                                            if (
                                              tbldata[j].rt === secData.dt[i].rt
                                            ) {
                                              if (
                                                tbldata[j].etin ===
                                                secData.dt[i].etin
                                  ...
                                  case "nil":
                                    if (form == "GSTR2") {
                                      logger.log(
                                        "info",
                                        "entered in nilsupplies section"
                                      );
                                      tbldata = gstfile.nil_supplies;
                                  ...
                                  case "ata":
                                    ...
                                    for (
                                      ...
                                      var arrayFound;
                                      if (form == "GSTR1") {
                                        //since now diff_percent is also a key
                                        arrayFound = tbldata.myFind({
                                          pos: secData.dt[i].pos,
                                          omon: secData.dt[i].omon,
                                          diff_percent:
                                  ...
                                  case "hsn":
                                    ...
                                    if (typeof tbldata.data == "object") {
                                      var total_hsn_objects =
                                        gstfile.hsn.data.length;
                                    } else {
                                      var total_hsn_objects = 0;
                                    }
                                    total_hsn_objects++; // num  should atleast be 1
                                    ...
                                    for (
                                      ...
                                      for (
                                        var j = 0;
                                        j < tbldata.data.length;
                                        j++
                                      ) {
                                        if (
                                          tbldata.data[j].hsn_sc ===
                                          secData.dt[i].data[0].hsn_sc
                                        ) {
                                          ...
                                          if (
                                            ...
                                            if (
                                              ...
                                              if (
                                                ...
                                                if (
                                                  tbldata.data[j].rt ==
                                                  secData.dt[i].data[0].rt
                                                ) {
                                                  secData.dt[i].data[0].num =
                                                    tbldata.data[j].num;
                                  ...
                                  case "b2bura": //for R2
                                    ...
                                    for (
                                      ...
                                      if (subarrayFound.length == 0) {
                                        var secdatalength =
                                          secData.dt[i].inv.length;

                                        tbldata[0].inv.push(
                                          secData.dt[i].inv[0]
                                        );
                                      } else {
                                        var index = tbldata.indexOf(
                                          subarrayFound[0]
                                        );
                                        tbldata.splice(index, 1); //delete row first with matched inv

                                        tbldata[0].inv.splice(
                                          index,
                                          0,
                                          secData.dt[i].inv[0]
                                        ); //insert updated row in the same index of previous row
                                      }
                                      logger.log(
                                        "info",
                                        "How many times am i repeated "
                                      );
                                    }
                                  ...
                                  case "hsnsum":
                                    ...
                                    for (
                                      ...
                                      for (
                                        var j = 0;
                                        j < tbldata.det.length;
                                        j++
                                      ) {
                                        if (
                                          tbldata.det[j].hsn_sc ===
                                          secData.dt[i].det[0].hsn_sc
                                        ) {
                                          ...
                                          if (
                                            ...
                                            if (
                                              ...
                                              secData.dt[
                                                i
                                              ].det[0].uqc.toLowerCase()
                                            ) {
                                              secData.dt[i].det[0].num =
                                                tbldata.det[j].num;
                            ...
                            if (jsonObj.length == 0) {
                              logger.log(
                                "info",
                                "No duplicate invoice found and data added successfully::%s",
                                jsonObj
                              );
          ...
          } //when user will come second time and directory is there to save the file.
          // if directory is there we will read the file and append the content into it.
          // if file is not there we need to create.
          else {
            fs.readFile(filename, "utf8", function (err, data) {
              // 1. read the file 2.Create file if not exist.  3.Append data if exist
              if (err) {
                ...
                fs.writeFile(
                  ...
                  formDataFormat,
                  function (
                    err //create a file since directory is there .
                  ) {
                    if (err) {
                      //error in creating a file
                      logger.log(
                        "error",
                        "error in creating a file:: %s",
                        err.message
                      );
                      callback(err, null);
                    } //file is created
                  ...
                  if (!secData.dt[0] && isFirstFlowImport) {
                    ...
                    switch (secData.cd) {
                      case "b2b":
                        ...
                        for (var i = 0; i < secData.dt.length; i++) {
                          if (
                            gstin == secData.dt[i].ctin ||
                            gstin == secData.dt[i].inv[0].etin
                          ) {
                            isSameGSTIN = gstin;
                          } else {
                            ...
                            if (arrayFound.length != 0) {
                              ...
                              if (subArrayFound.length == 0) {
                                ...
                                jsonObj.push(
                                  secData.cd + ":" + responseinvce[0].inum
                                );
                              }
                            } else {
                            ...
                            if (arrayFound.length != 0) {
                              ...
                              if (subArrayFound.length == 0) {
                                ...
                                if (subArrayFound2.length == 0) {
                                  ...
                                  jsonObj.push(
                                    secData.cd + ":" + responseinvce[0].oinum
                                  );
                                }
                              } else {
                                subarray.splice(subIndex, 1);
                      ...
                      case "doc_issue":
                        ...
                        if (type == "Import") {
                          if (!secData.dt.length && tbldata["flag"]) {
                            ...
                            if (tbldata["flag"]) delete tbldata["flag"]; //otherwise delete flag N which is not required in schema
                          }
                        }
                        break;
                      case "b2cl":
                          ...
                          if (gstin == secData.dt[i].ctin) {
                            ...
                            if (arrayFound.length != 0) {
                              ...
                              if (subArrayFound.length == 0) {
                                ...
                                jsonObj.push(
                                  secData.cd + ":" + responseinvce[0].nt_num
                                );
                              }
                            } else {
                              //If the first row in the excel doesn't carry CFS flag but the 2nd one does,
                            ...
                            if (arrayFound.length != 0) {
                              ...
                              if (subArrayFound.length == 0) {
                                ...
                                if (subArrayFound2.length == 0) {
                                  ...
                                  jsonObj.push(
                                    secData.cd + ":" + responseinvce[0].ont_num
                                  );
                                }
                              } else {
                                subarray.splice(subIndex, 1);
                      ...
                      case "atadja":
                        ...
                        for (var i = 0; i < secData.dt.length; i++) {
                          ...
                          if (arrayFound.length != 0) {
                            ...
                            tbldata.splice(Index, 0, secData.dt[i]);
                            jsonObj.push(
                              secData.cd + ":" + responseinvce[0].pos
                            );
                          } else {
                            tbldata.push(secData.dt[i]);
                          }
                        ...
                        for (var i = 0; i < secData.dt.length; i++) {
                          ...
                          for (var j = 0; j < tbldata.data.length; j++) {
                            if (
                              ...
                              if (
                                tbldata.data[j].uqc.toLowerCase() ===
                                secData.dt[i].data[0].uqc.toLowerCase()
                              ) {
                                if (
                                  !isCurrentPeriodBeforeAATOCheck(
                                    newHSNStartDateConstant,
                                  ...
                                  tbldata.data[j].desc.toLowerCase() ===
                                  secData.dt[i].data[0].desc.toLowerCase()
                                ) {
                                  secData.dt[i].data[0].num =
                                    tbldata.data[j].num;
                          ...
                          if (count != 1) {
                            ...
                            if (!secData.dt[i].data[0].hsn_sc) {
                              jsonObj.push(
                                secData.cd + ":" + secData.dt[i].data[0].desc
                              );
                            } else {
                              jsonObj.push(
                                secData.cd + ":" + secData.dt[i].data[0].hsn_sc
                              );
                            }
                          }
                        }
                      ...
                      case "b2bura": //for R2
                        ...
                        for (var i = 0; i < secData.dt.length; i++) {
                          ...
                          if (subarrayFound.length === 0) {
                            ...
                            jsonObj.push(
                              secData.cd + ":" + secData.dt[i].inv[0].inum
                            );
                          }
                        }
                        break;
                      ...
                      case "imp_ga": //for  R2
                        ...
                        for (var i = 0; i < secData.dt.length; i++) {
                          ...
                          for (var j = 0; j < tbldata.length; j++) {
                            if (
                              tbldata[j].port_code === secData.dt[i].port_code
                            ) {
                              if (
                                tbldata[j].boe_num === secData.dt[i].boe_num
                              ) {
                                tbldata.splice(j, 1);
                                tbldata.splice(j, 0, secData.dt[i]);
                                count = 1;
                          ...
                          if (count != 1) {
                            ...
                            jsonObj.push(
                              secData.cd +
                                ":" +
                                secData.dt[i].port_code +
                                "_" +
                                secData.dt[i].boe_num
                            );
                          }
                        }
                        //changes by prakash - end
                      ...
                      case "hsnsum": //for  R2
                        ...
                        for (var i = 0; i < secData.dt.length; i++) {
                          ...
                          if (tbldata.length !== 0) {
                            ...
                            for (var j = 0; j < tbldata.det.length; j++) {
                              if (
                                ...
                                if (
                                  tbldata.det[j].desc.toLowerCase() ===
                                  secData.dt[i].det[0].desc.toLowerCase()
                                ) {
                                  if (
                                    tbldata.det[j].uqc.toLowerCase() ===
                                    secData.dt[i].det[0].uqc.toLowerCase()
                                  ) {
                                    //todo
                                    secData.dt[i].det[0].num =
                                      tbldata.det[j].num;
                          ...
                          if (count != 1) {
                            ...
                            if (!secData.dt[i].det[0].hsn_sc) {
                              jsonObj.push(
                                secData.cd + ":" + secData.dt[i].det[0].desc
                              );
                            } else {
                              jsonObj.push(
                                secData.cd + ":" + secData.dt[i].det[0].hsn_sc
                              );
                            }
                          }
                        }
    ...
    logger.log(
      "error",
      "Unexpected error while adding the invoices:: %s",
      err.message
    );
    response.error(errorObject, res);
  } finally {
      ...
      [
        ...
        function (gstfileNew, callback) {
          ...
          fs.readFile(filename, "utf8", function (err, data) {
                ...
                switch (key) {
                  ...
                  case "txpda":
                    for (var i = 0; i < value.length; i++) {
                      if (value[i].error_msg == "M") {
                        ...
                        if (value[i].diff_percent != 0.65)
                          value[i].diff_percent = null;
                        if (key == "txpd") gstfileNew.atadj.push(value[i]);
                        else {
                          if (!gstfileNew[key]) gstfileNew[key] = [];
                  ...
                  case "nil":
                    if (value[0] != "undefined") {
                      if (
                        value[0]["error_msg"] != "undefined" &&
                        value[0]["error_cd"] != "undefined"
                      ) {
                    ...
                    errorData[key] = newValue;
                  case "nil_supplies":
                    if (typeof value[0] !== "undefined") value = value[0];
            ...
            res.on("finish", function () {
              del.sync([
                uploadedErrdir + gstin + "_" + form + "_" + fy + "_" + month,
              ]);
            });
            //changes made by prakash - end
          });
    ...
    async.waterfall(
      [
        function (callback) {
          ...
          logger.log(
            "info",
            "Entering Offline File:: setDeleteFlag with tbl_data :: %s",
            tbl_data
          );
          logger.log(
            "info",
            "Entering Offline File:: setDeleteFlag with tblcd :: %s",
            tblcd
          );
            ...
            if (err) {
              ...
              switch (tblcd) {
                ...
                case "b2ba":
                  ...
                  logger.log(
                    "info",
                    "Entering Offline File:: setDeleteFlag with b2b"
                  );
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    for (var index = 0; index < tbldata.length; index++) {
                      ...
                      if (subArrayFound.length != 0) {
                        for (
                          var fIndex = 0;
                          fIndex < subArrayFound.length;
                          fIndex++
                        ) {
                          if (
                            (((subArrayFound[fIndex].updby === "S" &&
                              subArrayFound[fIndex].cflag !== "R") ||
                              !subArrayFound[fIndex].updby) &&
                              form === "GSTR1") ||
                            (subArrayFound[fIndex].updby === "R" &&
                              form === "GSTR2")
                          ) {
                            var invIndex = subarray.indexOf(
                              subArrayFound[fIndex]
                            );
                            ...
                            if (subarray[invIndex]["chksum"]) {
                              ...
                              if (
                                (subarray[invIndex]["irn"] != "" &&
                                  subarray[invIndex]["irn"] != undefined) ||
                                (subarray[invIndex]["srctyp"] != "" &&
                                  subarray[invIndex]["srctyp"] != undefined) ||
                                (subarray[invIndex]["irngendate"] != "" &&
                                  subarray[invIndex]["irngendate"] != undefined)
                              ) {
                ...
                case "b2cla":
                  ...
                  logger.log(
                    "info",
                    "Entering Offline File:: setDeleteFlag with b2cl"
                  );
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    ...
                    if (arrayFound.length == 1) {
                      ...
                      if (tbldata[index]["chksum"]) {
                        ...
                        if (
                          (tbldata[index]["irn"] != "" &&
                            tbldata[index]["irn"] != undefined) ||
                          (tbldata[index]["srctyp"] != "" &&
                            tbldata[index]["srctyp"] != undefined) ||
                          (tbldata[index]["irngendate"] != "" &&
                            tbldata[index]["irngendate"] != undefined)
                        ) {
                ...
                case "atadj":
                  tbldata = gstfile.txpd;
                  if (!tbldata) tbldata = gstfile.atadj;
                  ...
                  if (form == "GSTR1") {
                    ...
                    tbldata = gstfile.atadj;
                    if (!tbldata) tbldata = gstfile.txpd;
                    for (var j = 0; j < invdltArray.length; j++) {
                      var arrayFound = tbldata.myFind({
                        pos: invdltArray[j].pos,
                      });
                ...
                case "ata":
                  if (tblcd == "atadja") {
                    tbldata = gstfile.txpda;
                    if (!tbldata) tbldata = gstfile.atadja;
                  } else {
                    tbldata = gstfile.ata;
                  }
      ...
      function (err, result) {
        ...
        if (err) {
          ...
          logger.log(
            "error",
            "Error While deleting the invoices :: %s",
            errorObject
          );
          response.error(errorObject, res);
        } else {
          logger.log(
            "info",
            "Document marked for delete successfully:: %s",
            result
          );
          response.success(result, res);
        }
    ...
    async.waterfall(
      [
        function (callback) {
          ...
          logger.log(
            "info",
            "Entering Offline File:: setFlagAll with tbl_data :: %s",
            tbl_data
          );
          logger.log(
            "info",
            "Entering Offline File:: setFlagAll with tblcd :: %s",
            tblcd
          );
          var invdltArray = req.body.invdltArray; // this will contain an array of objects.Each object will consist of unique keys to update
          var type = req.body.type;
            ...
            if (err) {
              ...
              switch (tblcd) {
                ...
                case "b2ba":
                  ...
                  logger.log(
                    "info",
                    "Entering Offline File:: setFlagAll with b2b"
                  );
                  ...
                  for (var index = 0; index < tbldata.length; index++) {
                    ...
                    for (
                      var invIndex = 0;
                      invIndex < invArr.length;
                      invIndex++
                    ) {
                      if (
                        (((invArr[invIndex].updby === "S" &&
                          invArr[invIndex].cflag !== "R") ||
                          !invArr[invIndex].updby) &&
                          form === "GSTR1") ||
                        (invArr[invIndex].updby === "R" && form === "GSTR2")
                      ) {
                        if (invArr[invIndex]["chksum"]) {
                          invArr[invIndex]["flag"] = "D";
                          if (
                            (invArr[invIndex]["irn"] != "" &&
                              invArr[invIndex]["irn"] != undefined) ||
                            (invArr[invIndex]["srctyp"] != "" &&
                              invArr[invIndex]["srctyp"] != undefined) ||
                            (invArr[invIndex]["irngendate"] != "" &&
                              invArr[invIndex]["irngendate"] != undefined)
                          ) {
                ...
                case "b2cla":
                  ...
                  logger.log(
                    "info",
                    "Entering Offline File:: setFlagAll with b2cl"
                  );
                ...
                case "cdnra":
                  ...
                  for (var index = 0; index < tbldata.length; index++) {
                    ...
                    for (
                      var invIndex = 0;
                      invIndex < ntArr.length;
                      invIndex++
                    ) {
                      if (
                        (((ntArr[invIndex].updby === "S" &&
                          ntArr[invIndex].cflag !== "R") ||
                          !ntArr[invIndex].updby) &&
                          form === "GSTR1") ||
                        (ntArr[invIndex].updby === "R" && form === "GSTR2")
                      ) {
                        if (ntArr[invIndex]["chksum"]) {
                          ntArr[invIndex]["flag"] = "D";
                          if (
                            (ntArr[invIndex]["irn"] != "" &&
                              ntArr[invIndex]["irn"] != undefined) ||
                            (ntArr[invIndex]["srctyp"] != "" &&
                              ntArr[invIndex]["srctyp"] != undefined) ||
                            (ntArr[invIndex]["irngendate"] != "" &&
                              ntArr[invIndex]["irngendate"] != undefined)
                          ) {
                  ...
                  for (var index = 0; index < tbldata.length; index++) {
                    if (tbldata[index]["chksum"]) {
                      ...
                      if (
                        ...
                        if (tbldata[index]["irn"]) delete tbldata[index]["irn"];
                        if (tbldata[index]["irngendate"])
                          delete tbldata[index]["irngendate"];
                        if (tbldata[index]["srctyp"])
                          delete tbldata[index]["srctyp"];
                ...
                case "hsn":
                  ...
                  for (
                    var index = tbldata.data.length - 1;
                    index >= 0;
                    index--
                  ) {
                    if (tbldata.data[index]["chksum"]) {
...
var deleteErrData = function (req, res) {
  ...
  try {
    async.waterfall(
      [
        function (callback) {
          ...
          logger.log(
            "info",
            "Entering Offline File:: deleteErrData with tbl_data :: %s",
            tbl_data
          );
          logger.log(
            "info",
            "Entering Offline File:: deleteErrData with tblcd :: %s",
            tblcd
          );
          var invdltArray = req.body.invdltArray; // this will contain an array of objects.Each object will consist of unique keys to update
          var type = req.body.type;
            ...
            if (err) {
              ...
              switch (tblcd) {
                ...
                case "b2ba":
                  ...
                  logger.log(
                    "info",
                    "Entering Offline File:: deleteErrData with b2b"
                  );
                ...
                case "b2cla":
                  ...
                  logger.log(
                    "info",
                    "Entering Offline File:: deleteErrData with b2cl"
                  );
                  ...
                  for (var i = 0; i < invdltArray.length; i++) {
                    if (
                      ...
                      tbldata.forEach(function (row, outerIndex) {
                        row.data.forEach(function (data, innerIndex) {
                          if (
                            data.hsn_sc == invdltArray[i].hsn_sc &&
                            data.desc == invdltArray[i].desc &&
                            data.uqc == invdltArray[i].uqc &&
                            data.num == invdltArray[i].num
                          ) {
      ...
      function (err, result) {
        ...
        if (err) {
          ...
          logger.log(
            "info",
            "Return document deleted successfully:: %s",
            result
          );
    ...
    logger.log(
      "error",
      "Unexpected error while deleting the data:: %s",
      err.message
    );
    response.error(errorObject, res);
  } finally {
    ...
    async.waterfall(
      [
        function (callback) {
          ...
          logger.log(
            "info",
            "Entering Offline File:: updateImport with tbl_data :: %s",
            tbl_data
          );
          logger.log(
            "info",
            "Entering Offline File:: updateImport with tblcd :: %s",
            tblcd
          );
          var invdltArray = req.body.invdltArray; // this will contain an array of objects.Each object will consist of unique keys to update
          var type = req.body.type;
            ...
            if (err) {
              ...
              switch (tblcd) {
                ...
                case "b2ba":
                  ...
                  logger.log(
                    "info",
                    "Entering Offline File:: updateImport with b2b"
                  );
                ...
                case "b2cl":
                  ...
                  logger.log(
                    "info",
                    "Entering Offline File:: updateImport with b2cl"
                  );
                ...
                case "cdnur":
                case "cdnura":
                  if (tblcd == "cdnur") tbldata = gstfile.cdnur;
                  else tbldata = gstfile.cdnura;
            ...
            if (err) {
              ...
              switch (tblcd) {
                ...
                case "cdnura":
                  ...
                  break;
                case "nil":
                  if (form == "GSTR1") gstfile.nil = { inv: [] };
                  else gstfile.nil_supplies = {};
                ...
                case "txia":
                  ...
                  break;
                case "atadj":
                  if (gstfile.txpd) gstfile.txpd = [];
                  if (gstfile.atadj) gstfile.atadj = [];
              ...
              callback(
                null,
                "Records of the selected section have been removed from the tool successfully!"
              );
            }
          });
        },
    ...
    async.waterfall(
      [
        function (callback) {
          logger.log(
            "info",
            "entered in async.waterfall function 0, checking for data-cache"
          );
          if (dataObj.cache_key != undefined) {
            myCache.get(dataObj.cache_key, function (err, value) {
              if (!err) {
                  ...
                  switch (secData.cd) {
                            ...
                            if (tbldata[j].pos === oldPos) {
                              if (tbldata[j].rt === oldRate) {
                                if (tbldata[j].etin === oldEtin) {
                                  if (
                                    tbldata[j].diff_percent === old_diffprcnt
                                  ) {
                                    tbldata.splice(j, 1);
                                    tbldata.splice(j, 0, secData.dt[i]);
                                    jsonObj.push(
                          ...
                          if (arrayFound.length) {
                            for (var j = 0; j < arrayFound.length; j++) {
                              ...
                              if (subArrayFound.length == 1) {
                                ...
                                tbldata.splice(j, 0, secData.dt[i]);
                                jsonObj.push(
                                  secData.cd + ":" + secData.dt[i].nt[0].nt_num
                                );
                              }
                            }
                          }
                    ...
                    case "cdnur":
                      ...
                      for (var i = 0; i < secData.dt.length; i++) {
                        ...
                        if (old_ntnum) keyObj.nt_num = old_ntnum;
                    ...
                    case "at":
                    case "atadj":
                      if (secData.cd == "at") tbldata = errorData.at;
                      else tbldata = errorData.txpd;
                    ...
                    case "ata":
                    case "atadja":
                      if (secData.cd == "ata") tbldata = errorData.ata;
                      else tbldata = errorData.txpda;
                    ...
                    for (var k = 0; k < gstfile[tblcd].length; k++) {
                      if (gstfile[tblcd][k].ctin == tbl_data[i].ctin)
                        for (var j = 0; j < gstfile[tblcd][k].inv.length; j++) {
                          if (
                            gstfile[tblcd][k].inv[j].inum == tbl_data[i].inum
                          ) {
                    ...
                    for (var j = 0; j < gstfile[tblcd].length; j++) {
                      if (gstfile[tblcd][j].ctin == tbl_data[i].ctin) {
                        for (var k = 0; k < gstfile[tblcd][j].nt.length; k++)
                          if (
                            gstfile[tblcd][j].nt[k].inum == tbl_data[i].inum
                          ) {
...
function findAndSwapCFSInv(secData, currIndex) {
  ...
  if (secName == "b2b" || secName == "cdnr") {
    ...
    for (var i = currIndex + 1; i < secData.dt.length; i++) {
      if (
        secData.dt[i].ctin == currCtin &&
        secData.dt[i].hasOwnProperty("cfs")
      ) {
...
function loadHSNData(req, res, err) {
  ...
  try {
    var fileData = fs.readFileSync(
      __dirname + "/../public/data/HSN.json",
      "utf8"
    );
    fileData = JSON.parse(fileData);
    var result = { data: [] };
    if (searchtext != null) {
      fileData.data.forEach((obj, index) => {
        ...
        if (pattern.test(searchtext)) {
          indexOfN =
            obj.c.substring(0, searchtext.length) == searchtext ? 0 : -1;
        } else {
